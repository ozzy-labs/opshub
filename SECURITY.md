...
### Phase 21 browser read layer — what changed

Phase 21 ([ADR-0037](docs/adr/0037-browser-read-layer-playwright.md) + [ADR-0010](docs/adr/0010-connector-contract.md) §Phase 21 revision (n)/(o) + [ADR-0022](docs/adr/0022-mcp-server-surface.md) §決定 (g)) adds a Playwright-backed **browser read layer** — an 11th connector (`web`, `opshub web sync`) and an MCP write tool (`browser.fetch`) — that opens **two new network-egress paths** the threat model must account for:

- **New egress surface (the browser fetch path).** Until Phase 21, every connector reached a *known SaaS API* the operator had explicitly authenticated; `recall.search` / `search` / other read MCP tools touched only local SQLite. The browser layer renders arbitrary operator-listed `http` / `https` URLs (`web` connector) or an ad-hoc `url` (`browser.fetch`). This is egress to **arbitrary Web origins**, leaving a trace in each target's access log and (for authenticated reverse proxies / internal tools) potentially carrying ambient credentials of the workstation. Operators choose what to render — the connector fetches **only the URLs in `[connectors.web] pages`** and never follows links / sitemaps (ADR-0010 §Phase 21 revision (n)), and `browser.fetch` accepts only `http` / `https` (the handler rejects `file` / `data` / `ftp` / `javascript` so a host LLM cannot turn the headless browser into a local-file read primitive).
- **`browser.fetch` is classified write-category precisely to preserve the MCP boundary invariant.** It returns data but **egresses the public network**, so it sits in the same HITL bucket as `connector.sync` (`readOnlyHint=false` / `destructiveHint=true` / `openWorldHint=true`, [ADR-0037](docs/adr/0037-browser-read-layer-playwright.md) §決定 (e), [ADR-0022](docs/adr/0022-mcp-server-surface.md) §決定 (g)). The "read tool = local SQLite only, auto-approve OK" invariant is preserved: a host honouring the annotations requires human confirmation before every fetch. Pinned by `tests/unit/mcp/test_registry_policy` (the surface is exactly 13 read + 6 write) and `tests/unit/mcp/test_browser_fetch_handler.py`.
- **Web page bodies are an indirect-prompt-injection carrier.** Rendered DOM text lands in `sources.body` tagged `provenance_origin="external"` + `provenance_trust="untrusted"` ([ADR-0020](docs/adr/0020-full-local-content-retention.md) §(e)) — same poisoning-mitigation contract as Slack / GitHub / Office bodies. The mitigations are the existing three layers (encryption at rest, provenance tags, append-only rebuild from the event log) **plus** two structural choices: `browser.fetch` is HITL per call (no auto-fetch), and operations (click / fill / submit) are deferred (ADR-0037 §決定 (f)) so the "read a poisoned body → act on its instructions" chain has no code path.
- **Profile isolation.** The browser core uses an **opshub-dedicated user-data-dir** (`default_data_dir() / "browser"`); the operator's everyday Chrome profile (cookies, extensions, saved logins) is never touched (ADR-0037 §決定 (c)). The `cdp_endpoint` (`connect_over_cdp`) escape hatch — attaching to an operator-launched Chrome with `--remote-debugging-port` — is wired but reserved for a future authenticated-session story; opening a debug port is an operator decision with its own local attack surface and is off by default.
- **External write-back remains forbidden** ([ADR-0010](docs/adr/0010-connector-contract.md) §禁止事項 7). The browser read layer is **read-only** — no `page.click` / `page.fill` / `page.set_input_files` / form-submit callable exists in the connector or the `browser.fetch` handler. The `forbidden_callables` structural guard covers the `web` connector too; adding a browser write / operations path without a separate ADR (settling HITL approval granularity + prompt-injection defence + the ADR-0010 write-back ban整理) is a security regression.
- **No new at-rest secret.** Public Web pages need no auth, so the `web` connector carries no token / keyring slot. `browser.fetch` accepts no token argument (ADR-0022 §決定 (b)), and its returned snippet is run through `opshub.mcp._redact.redact_secrets` like every other tool return.
