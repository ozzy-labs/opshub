...
Phase 12 (2026-05-31) widened the assistant skill repertoire from 5 to **14** (10 read / 4 HITL write); Phase 25-E added `catchup` to make it **15** (11 read / 4 HITL write). The catalog below shows the most common triggers; the full responsibility map lives in [`docs/assistant-agent.md`](docs/assistant-agent.md).
...
| "What changed since I last looked?" / "前回見て以降どうなった" | `catchup` (Phase 25-E) | Priority-ordered diff since the seen-marker (new sources + overdue commitments + unhandled Slack demand); advances the marker — pair of personal-brief |

The 15 assistant skills live under [`docs/skills/<name>/SKILL.md`](docs/skills/) (Phase 12 H1 made opshub the SSOT). The original 5 skills were renamed (`daily-brief` → `personal-brief`, `file-lookup` → `find-document`); the other 9 are new in Phase 12 H2-H5, and `catchup` is new in Phase 25-E. The distribution channel was pinned in Phase 16-A ([ADR-0029](docs/adr/0029-distribute-assistant-skills-via-opshub-package.md)) to **opshub Python package bundling + `opshub skills install`** (landed in Phase 16-B/C/D); see [Install the assistant skills](#install-the-assistant-skills) below. The `@ozzylabs/skills` Renovate preset (handbook ADR-0016) keeps shipping ecosystem-common skills (drive / lint / commit / ...); the assistant 15 are carved out into the opshub-bundled channel (disjoint namespaces, ADR-0029 §決定 (h)). See [`docs/assistant-agent.md`](docs/assistant-agent.md) for the responsibility map, MCP tool dependency matrix, pair structure, HITL boundary, and what OpsHub deliberately does not do (no write-back to SaaS, no always-on runtime, no auto-apply).
...
Phase 16-A ([ADR-0029](docs/adr/0029-distribute-assistant-skills-via-opshub-package.md)) confirmed package bundling + `opshub skills install` as the canonical distribution channel for the 15 assistant skills (SSOT remains opshub `docs/skills/<name>/SKILL.md`). Phase 16-B ([#383](https://github.com/ozzy-labs/opshub/issues/383)) shipped the CLI; Phase 16-C ([#384](https://github.com/ozzy-labs/opshub/issues/384)) wired it into `opshub init` so the standard 2-step setup also installs the 15 assistant skills:

```bash
uv tool install ozzylabs-opshub[mcp]
opshub init   # MCP server init + 15 assistant skills install (TTY prompt; non-interactive default = install)
```
...
| 25 | 秘書化 v1 (commitment ledger + person axis + catchup) | Two-way **commitment ledger** (ADR-0042) mines promises you made (`i_owe`) and requests you're waiting on (`owed_to_me`) from the data opshub already holds — including your *sent* side. Manual on-demand `opshub commitment scan` runs an LLM extraction pass (`CommitmentExtracted` events, deterministic projection — no LLM in replay, ADR-0002); browsing (`commitment list`) needs no LLM. **Person axis** (ADR-0043) resolves Slack `U...` / email / GitHub login across connectors into one node (exact auto-merge / fuzzy HITL merge). **catchup** (`opshub catchup`) surfaces only the diff since the seen-marker. Built on the Phase 25-A cross-cutting author normalisation (ADR-0010 §revision: `author_handle` + per-connector operator self-identity). New CLIs `commitment` / `person` / `catchup`; migrations `0034`–`0039`; MCP surface 19 → **27 tools** (15 read + 12 write, ADR-0022 §決定 (h); `catchup` advances the seen marker so it is a non-destructive write); skill catalog 14 → **15** (`catchup`). Reminders stay read-only (no external nudge, ADR-0010 §禁止事項 7) |
