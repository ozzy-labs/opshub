...
**Status**: Phase 1 (foundation) + Phase 2 (coordination) + Phase 3 (connector layer + workspace ingest、MVP = framework + GitHub) + Phase 4 (semantic recall layer、MVP = Pluggable Embedder + sqlite-vec + recall + 重複検出) complete (2026-05-17) — event store、全 projection (tasks / inbox_items / decisions / work_sessions / agent_runs / locks / handoffs / sources / connector_cursors / ingested_files / embeddings)、CLI (`init` / `task` / `inbox` / `decision` / `lock` / `session` / `agent run` / `handoff` / `connector auth set` / `connector sync` / `connector list` / `workspace ingest` / `workspace generate` / `projections rebuild` / `embeddings rebuild` / `embeddings status` / `embeddings find-duplicates` / `recall` / `db migrate`)、markdown 生成、GitHub connector、workspace inbox file ingest、Pluggable Embedder (local sentence-transformers / OpenAI / Voyage の 3 backend) + sqlite-vec backed VectorStore が end-to-end で動作。Slack / Microsoft 365 / Box の connector は Phase 3.x、briefing 自動生成 / `links` projection 本実装 / event 駆動自動 embed は Phase 5 で順次追加 (詳細は `docs/principles.md` §9)。
