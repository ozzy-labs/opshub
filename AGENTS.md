...
**Status**: Phase 1 (foundation) + Phase 2 (coordination) + Phase 3 (connector layer + workspace ingest、MVP = framework + GitHub) complete (2026-05-17) — event store、全 projection (tasks / inbox_items / decisions / work_sessions / agent_runs / locks / handoffs / sources / connector_cursors / ingested_files)、CLI (`init` / `task` / `inbox` / `decision` / `lock` / `session` / `agent run` / `handoff` / `connector auth set` / `connector sync` / `connector list` / `workspace ingest` / `workspace generate` / `projections rebuild` / `embeddings` / `db migrate`)、markdown 生成、GitHub connector、workspace inbox file ingest が end-to-end で動作。Slack / Microsoft 365 / Box の connector は Phase 3.x で順次追加。次は Phase 4 (semantic layer: vector recall / 重複検出 / briefing 自動生成) の設計に着手 (詳細は `docs/principles.md` §9)。
