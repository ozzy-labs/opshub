"""Google Calendar connector settings (re-export shim).

The canonical :class:`GoogleCalendarConnectorSettings` lives in
...
the canonical class under
``opshub.connectors.google_calendar.settings`` so that:

1. Phase 14 plan §3 Sub-issue G4's "5 module 構成 (cursor + client +
   mapper + connector + settings)" reads consistently with the on-disk
   module list.
...
from opshub.core.config import GoogleCalendarConnectorSettings

__all__ = ["GoogleCalendarConnectorSettings"]
