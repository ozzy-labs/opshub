...
共通方針は AGENTS.md を参照（Phase 1-18 完了状態の記載含む）。以下は Claude Code 固有の設定。
...
長時間 CLI (`opshub <connector> sync` / `opshub slack conversations` / `opshub embeddings rebuild` / `opshub embeddings drain` / `opshub projections rebuild`) は TTY 時に進捗を自動表示し、`--progress` / `--no-progress` フラグまたは `OPSHUB_PROGRESS` 環境変数 (truthy = `1`/`true`/`yes`/`on`、falsy = `0`/`false`/`no`/`off`、case-insensitive) で上書きできる ([ADR-0026](docs/adr/0026-cli-progress-reporting.md))。`opshub slack conversations` は `--since` 指定時に activity 軸の probe を起動する: Phase 19-B ([ADR-0034](docs/adr/0034-slack-engagement-axis.md)) で default が `--activity=mine` (engagement 軸) に切替わり、spinner 説明文が `"listing conversations + engagement"`、table の追加列が `LAST_POST` (`YYYY-MM-DD` UTC、`last_self_post_ts` を表示)、TOML コメントが `"last post YYYY-MM-DD"` 形式になる。1 度だけ stderr に `notice: search.messages may lag by minutes; use --activity=any for live activity.` が出る (indexing lag advisory、ADR-0034 §(i)、`-q` / `OPSHUB_LOG_LEVEL` の影響を受けない、完全 suppress したい場合は `--activity=any` を明示)。`--activity=mine` は User Token + `search:read` scope を要求し、Bot Token は `ConfigError` で拒否される (`--activity=any` への切替を誘導する)。`--all + --activity=mine` (= `--all` + default、または `--all` + `--since 30d`) も互いに非両立 (engagement 軸は self-member channel しか index しないため) で `ConfigError` で exit 1 する。`--activity=any` を明示すると Phase 17 #374 と同じ挙動に戻る (`LAST_ACTIVITY` 列 / `last_activity_ts` field / `"listing conversations + activity"` spinner、`*:history` scope 要求)。JSON 出力は populated 軸の field のみ emit する (どちらの軸でも他方の field は drop される)。`opshub connector` group は Phase 17 で全廃止 (ADR-0031、`docs/upgrading.md` §Phase 17 で旧 → 新コマンドの対応表を参照)。
