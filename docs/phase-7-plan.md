...
4. **Token storage** — `core/secrets` + keyring (ADR-0014) を再利用。key 規約 `connector:<name>:<purpose>` (例: `connector:slack:token`, `connector:ms365:refresh_token`, `connector:box:refresh_token`)。env var override `OPSHUB_CONNECTOR_<NAME>_<PURPOSE>` (例: `OPSHUB_CONNECTOR_SLACK_TOKEN`)。Slack の `purpose` は `token` (principal-neutral) — User Token / Bot Token のどちらも同じ slot を使う、Phase 7.x で ADR-0018 採択後の正規化形 (Phase 7 sub-A 実装時点では `bot_token` だったが、本 plan ドキュメントは現行 contract で記述)
...
| # | Commit | 概要 | Sub-issue |
|---|---|---|---|
| A1 | `feat(connectors/slack): auth + extras` | `[connectors-slack]` extras (`slack-sdk>=3.27` or pure httpx — slack-sdk 採択推奨、token management + auth.test の SDK helper が使える)。`src/opshub/connectors/slack/__init__.py` + `src/opshub/connectors/slack/auth.py` 新設: `SlackAuth.token -> str` property (`core/secrets.get_secret("connector:slack:token")` + `OPSHUB_CONNECTOR_SLACK_TOKEN` env override、Phase 7.x ADR-0018 で旧 `bot_token` から rename)。`auth.test_token() -> dict` で token validity check (返り値に `principal: "user" \| "bot"` を含む、ADR-0018 surface contract)。`opshub connector auth set connector:slack` 経路が `connector:slack:token` で keyring 保存可能であることを test pin (Phase 5 A5 で generic 化済の auth CLI を使う、新 CLI subcommand は不要) | A |
