...
Phase 8 sub-issue A-E の実装で本 ADR の決定 (a)-(h) は以下のとおり pin する想定 (Phase 8 E1 closeout 時点で全て pinned):

- **(a) `links` projection schema** — `src/opshub/projections/links.py` に `links_table` 定義を Phase 8 A2 で追加 + migration 0016 で create。UNIQUE 制約 + 2 INDEX 作成は `tests/unit/projections/test_links_schema.py` で metadata inspection 経由で pin。`alembic upgrade head` で table + index が作成されることを `tests/integration/test_migrations.py` (既存) で再現
- **(b) `link_type` 初期 enum** — `src/opshub/projections/links.py` で `LinkType = Literal["applied_to", "referenced_in_briefing", "generated_from_briefing", "references", "manual"]` を Phase 8 B2 で定義、auto-extraction projector の出力は全てこの enum に collapse することを `tests/unit/projections/test_links_extractor.py` で pin。manual path の enum 外 warning は `tests/unit/cli/test_link.py` で stderr capture で pin
- **(c) Auto-extraction projector が新 event を発行しない** — `LinksProjector.apply` が EventStore に対して `append` を呼ばないことを `tests/unit/projections/test_links_extractor.py::test_extractor_does_not_emit_events` で Mock 経由で pin (`MagicMock` の `append.assert_not_called()` パターン)
- **(d) Manual link CRUD via `LinkCreated` / `LinkDeleted`** — `LinkService.add_link()` / `LinkService.remove_link()` が `LinkCreated` / `LinkDeleted` event を 1 件ずつ append することを `tests/unit/services/test_link_service.py` で pin。`LinkDeleted.reason` が `core.sanitise.sanitise_error_message` 経由になることは同 file で API key shaped 文字列を reason に渡して mask されることを pin
- **(e) Traversal depth limits + cycle detection** — `tests/unit/services/test_link_service.py` で:
  - `related(entity)` が 1-hop で outgoing + incoming 両方を返すこと
  - `trace(entity, depth=11)` が `ConfigError("depth 11 exceeds max 10")` を raise
  - `expand(entity, depth=6)` が `ConfigError("depth 6 exceeds max 5")` を raise
  - A → B → A graph で `expand(A, depth=2)` が無限ループしないこと (visited set による cycle break)
- **(f) `--expand-graph` flag default off** — `tests/integration/test_phase5_lifecycle.py` / `test_phase6_lifecycle.py` 等の Phase 5/6 既存 snapshot test を Phase 8 完了後も passing で pin (backward compat)。`--expand-graph` 指定時に graph 拡張 entity が prompt に含まれることは `tests/unit/services/test_briefing_service.py::test_generate_with_expand_graph_includes_neighbours` / `test_proposal_service.py::test_generate_with_expand_graph_includes_neighbours` で pin
...
- **(h) `LinkDeleted` = Hard delete** — `LinksProjector` が `LinkDeleted` event を消費後、`links` table に対応 row が **存在しないこと** を `tests/unit/projections/test_links_extractor.py::test_link_deleted_removes_row` で pin (SELECT で empty result を assert)。`links` table の schema (§決定 (a)) に `deleted_at` column が存在しないことを Phase 8 A2 で metadata inspection test で pin

end-to-end の整合確認は `tests/integration/test_phase8_lifecycle.py` (Phase 8 E1 で追加: `opshub task add` → `opshub embeddings rebuild` → `opshub brief` → `opshub propose generate` → `opshub propose apply` → `opshub graph trace <new-task-id>` で proposal → briefing → source の chain) + `tests/integration/test_phase8_manual_link_lifecycle.py` (manual `link add` / `link remove` round-trip + 冪等性) + `tests/integration/test_phase8_expand_graph_lifecycle.py` (`--expand-graph` で graph 拡張 entity が prompt に乗る) + `tests/integration/test_phase8_rebuild_idempotency.py` (`projections rebuild` で全 historical event から links graph が完全再構築) の 4 ファイルで pin する。
...
- [Architecture (§2.10 Knowledge graph layer は Phase 8 E1 で追記予定)](../architecture.md)
