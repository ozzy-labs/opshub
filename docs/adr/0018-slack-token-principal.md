...
7. **MVP scope**: `channels:history` / `channels:read` / `users:read` (3 scope) を User Token Scopes として登録する前提に書き換え。optional scope menu として `groups:history` (private) / `im:history` (DM) / `mpim:history` (MPIM) / `search:read` (search) / `files:read` (files) を README に記載
   - **`search:read` の利用条件** ([ADR-0034](0034-slack-engagement-axis.md) §(c) §(d) で格上げ、[ADR-0035](0035-slack-sort-axis-consolidation.md) §(d) §(f) で `--sort` 表記に rename): engagement 軸 (`opshub slack conversations --since` の `--sort=last_self_post` 経路、または `--sort=name + --since` 単独時の暗黙 engagement default 経路、`search.messages?query=from:@me`) を使う場合に必要。MVP scope セット (`channels:history` / `channels:read` / `users:read`) には含めず、engagement 軸を使う operator のみが追加で承認する設計
   - **Bot Token は `search:read` を構造的に持てない** (§Context Bot Token の構造的制約 を再掲) ため、Bot Token 運用 operator は engagement 軸利用不可。`--sort=last_activity` fallback で従来挙動 (broadcast 含む最終 activity) を選ぶ。ADR-0034 §(d) §(e) で明示エラー経路 (`ConnectorFailedError` exit 1) を pin、ADR-0035 §(f) で本 ADR §決定 7 と整合する形に表記を更新
...
- [ADR-0034: Slack Engagement Axis (Self-Posted Last Activity)](0034-slack-engagement-axis.md) — engagement 軸 (`opshub slack conversations --sort=last_self_post`、自分の最終発言 ts; Phase 23-G [#537](https://github.com/ozzy-labs/opshub/issues/537) で `--sort=last_self_post` 明示のみに直交化、旧 `--sort=name + --since` 暗黙 default は撤去) は `search.messages?query=from:@me` 経由で実装され、本 ADR §決定 7 で optional 列挙済の `search:read` scope を「engagement 軸利用時に必要」と格上げする。Bot Token は本 ADR §Context の構造的制約により `search:read` を持てず、engagement 軸利用不可 (ADR-0034 §(d) §(e) で明示エラー経路を pin)
- [ADR-0035: Slack Sort Axis Consolidation](0035-slack-sort-axis-consolidation.md) — ADR-0034 の CLI surface (`--activity={mine|any}`) を `--sort=name|last_self_post|last_activity` に部分 supersede し、本 ADR §決定 7 の `--activity=mine` 表記も `--sort=last_self_post` 経路に rename される。`search:read` 利用条件 / Bot Token 不可の decision 本質は ADR-0034 から完全継承 (ADR-0035 §(f))
- [ADR-0039: Slack Single-Workspace Non-Goal](0039-slack-single-workspace-non-goal.md) — principal 軸 (User/Bot) とは**直交**する制約として「1 install = 1 workspace」を pin していた (token slot `connector:slack:token` は principal を問わず単一 workspace 前提、`team_id` bind guard で enforce)。**Phase 24 [ADR-0041](0041-slack-multi-workspace.md) が ADR-0039 を supersede** し `1 install = N workspace` を first-class 化 — token slot は per-alias `connector:slack:<alias>:token` に分割され、bind guard は per-alias に一般化された。principal 軸との直交性は不変 (User/Bot token はいずれも各 alias の slot に格納される)
- [ADR-0040: Slack feature→scope SSOT + auth-test readiness](0040-slack-feature-scope-ssot-and-readiness.md) — 本 ADR §決定 (7) が散文で列挙する feature→scope 対応の **コード SSOT が `src/opshub/connectors/slack/scopes.py` (`FEATURE_SCOPES`) に移った**。§決定 (7) は decision 史として残すが、scope 真偽の参照先は scopes.py が canonical (ADR は data を二重に持たない)。`opshub slack auth test` の `features:` readiness ブロックも scopes.py 由来
