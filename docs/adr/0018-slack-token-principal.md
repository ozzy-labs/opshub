...
- [ADR-0039: Slack Single-Workspace Non-Goal](0039-slack-single-workspace-non-goal.md) — principal 軸 (User/Bot) とは**直交**する制約として「1 install = 1 workspace」を pin する。token slot `connector:slack:token` は principal を問わず単一 workspace 前提で、ADR-0039 が `team_id` bind guard でこれを loud に enforce する
