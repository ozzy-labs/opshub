...
本 ADR の決定 (a)-(i) は Phase 9 sub-issue A-C の実装で pin される (本 PR 時点ではプレースホルダ、Phase 9 C1 closeout PR で実テストファイル名を以下に追記する):

- **(a) `Connector` Protocol 再利用 + auth layer を OS 依存に置換** — B2 PR で `BoxDriveConnector` が `Connector` Protocol に適合することを `tests/unit/connectors/box_drive/test_connector.py` で pin (signature 一致 + `ConnectorContext` 受領 + `SourceService.observe` 経由 append)。`opshub connector auth set box_drive` の actionable error は `tests/unit/cli/test_connector_auth.py` で pin (C1 PR で追記)
- **(b) `os.stat()` のみ、`open()` 禁止** — B1 PR で `tests/unit/connectors/box_drive/test_scanner.py` の `test_scanner_does_not_call_open` (`unittest.mock.patch("builtins.open", side_effect=AssertionError("forbidden"))` を scan 全 path に適用) で pin。CldAPI non-hydration contract test は `tests/integration/test_box_drive_no_hydration.py` を `OPSHUB_BOX_DRIVE_TEST_ROOT` env opt-in で配置 (real env で持続検証)
- **(c) Identity = `rel_path`** — B2 PR で `tests/unit/connectors/box_drive/test_mapper.py::test_external_id_equals_rel_path` で pin。rename = 旧停止+新発火 制限は C1 PR の `tests/integration/test_phase9_lifecycle.py` で 2-pass scan (1 pass で `old.md`、2 pass で `old.md` → `new.md` rename 後) を再現し、`sources` projection に 2 row (旧停止 + 新発火) が残ることを pin
- **(d) Diff detection via `sources.fingerprint`** — A2 PR で migration 0017 が `sources.fingerprint TEXT NULL` 列を追加することを `tests/integration/test_migrations.py` で pin。`SourceObserved.fingerprint` field 追加は `tests/unit/domain/test_events_source.py` で pin (schema_version=1 据え置き + backward-compat)。`SourcesProjection` projector が `fingerprint=None` で NULL 書き込みすることを `tests/unit/projections/test_sources.py` で pin。`BoxDriveScanner.scan()` が prior_fingerprints と比較して変更ありのみ yield することは `tests/unit/connectors/box_drive/test_scanner.py::test_scanner_skips_unchanged_files` で pin
- **(e) 削除追跡なし** — C1 PR の `tests/integration/test_phase9_lifecycle.py` で 2-pass scan の 2 pass 目で file 削除を再現し、削除された file について `SourceObserved` / `SourceDeleted` のいずれも発火しないこと + `sources` projection に prior row が残ることを pin
- **(f) `root_path` platform-aware default** — B1 PR で `tests/unit/core/test_platform.py` で `detect_platform()` が WSL2 / macOS / linux / unsupported を正しく返すこと (mock based)、`box_drive_default_root_path()` が WSL2=`/mnt/b` / macOS=`~/Box` 展開 / Linux native=None を返すことを pin
- **(g) Excludes は `opshub.toml` inline** — B2 PR で `BoxDriveConnectorSettings.exclude_globs` field 定義を `tests/unit/core/test_config.py` で pin。`BoxDriveScanner` が exclude_globs を適用することを `tests/unit/connectors/box_drive/test_scanner.py::test_scanner_applies_exclude_globs` で pin
- **(h) Operator precondition は opshub 範囲外** — C1 PR で `docs/box-drive-setup.md` が新設され WSL2 / macOS / Linux native の手順を記載することを docs review で pin。`root_path` 不存在時の `ConfigError` は B2 PR の `tests/unit/connectors/box_drive/test_connector.py::test_connector_fails_fast_when_root_path_missing` で pin
