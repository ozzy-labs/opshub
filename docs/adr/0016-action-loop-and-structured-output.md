...
Phase 6 sub-issue C1 (closeout) で以下の test 配置を契約とする。本 ADR §決定 (a)-(h) の pin を test ファイル参照で記録する (実装は Phase 6 A2-A4 / B1-B4 で進行、本 ADR Validation セクションは C1 で実 path を埋める)。

- **(a) Structured output mechanism** — Anthropic / OpenAI / Ollama 3 client の `complete_structured` round-trip test (`tests/unit/llm/test_anthropic_client.py` / `tests/unit/llm/test_openai_client.py` / `tests/unit/llm/test_ollama_client.py`)。SDK / httpx を mock し、Pydantic schema → JSON schema → tool_use/tools → response parse → Pydantic instance の経路を検証
- **(b) Schema SSOT = Pydantic v2** — `opshub.llm.schema.pydantic_to_tool_schema` の test (`tests/unit/llm/test_schema.py`) で Pydantic v2 model → JSON schema 変換 + 3 backend が受け入れる shape (round-trip) を pin
- **(c) Human-in-the-loop** — apply は CLI 経由でのみ trigger される negative test (`tests/integration/test_phase6_lifecycle.py`)。`opshub propose --auto-apply` のような CLI surface が **存在しない** ことを assert (`typer` の app inspection で `propose` subcommand 一覧を取得し `apply` / `reject` / `generate` / `list` のみを許可)
- **(d) Idempotent apply** — `ProposalService.apply` を同 `(proposal_id, candidate_index)` で 2 回呼び 2 回目が `OpsHubError("candidate already <state>")` を raise する test (`tests/integration/test_phase6_propose_atomicity.py`)。`reject` 後の `apply` も同様
- **(e) Candidate types MVP** — `Candidate.kind: Literal["task", "decision"]` の Pydantic discriminator が `"inbox_item"` / `"source"` を **rejects** する unit test (`tests/unit/domain/events/test_proposal_candidate.py`)
- **(f) Schema versioning** — `Candidate.schema_version: Literal["v1"]` の default + validation test (`tests/unit/domain/events/test_proposal_candidate.py`)。Phase 6.x で `Literal["v1", "v2"]` 拡張時の両 version 読み分け test は Phase 6.x で追加
- **(g) Apply path through existing services** — `ProposalService.apply` 内部で `TaskService.add_task` / `DecisionService.record_decision` が呼ばれることを mock で pin (`tests/unit/services/proposals/test_service.py::test_apply_routes_through_task_service` / `test_apply_routes_through_decision_service`)。`TaskCreated` event が **`ProposalApplied` と同 UoW で append される** atomicity を `tests/integration/test_phase6_propose_atomicity.py` で検証
- **(h) Ollama daemon-absent fail-fast** — `OllamaLLMClient.__init__` で daemon URL に `httpx` を投げて connection refused / 404 が返るときに `ConfigError` を raise する test (`tests/unit/llm/test_ollama_client.py::test_init_raises_configerror_when_daemon_unreachable`)。`httpx_mock` で response を制御
- **principles.md §1 alignment** — `[llm] backend = "ollama"` 設定下の `opshub propose generate` が **localhost 以外への network 呼び出しを行わない** ことを `respx` / `httpx_mock` の "no unexpected request" 設定で pin (`tests/integration/test_phase6_ollama_lifecycle.py`)

C1 closeout 時に本セクションを Phase 5 D1 の Validation セクション (ADR-0015) と同密度で実 test path に置換する。
