...
1. **MCP tool 面** (§2.11a)
2. **秘書 14 Skill** (Phase 12 で 5 → 14 拡張、`docs/skills/<name>/SKILL.md` を opshub SSOT として保持。配信経路は Phase 16-A ([ADR-0029](adr/0029-distribute-secretary-skills-via-opshub-package.md)) で **opshub package 同梱 + `opshub skills install`** に確定。SSOT 位置は [ADR-0004 §決定 (c)](adr/0004-agent-runtime-boundary.md) で変更なし)。Skill catalog SSOT は [docs/secretary-agent.md](secretary-agent.md) (ADR-0004 §決定 (c-2))。
