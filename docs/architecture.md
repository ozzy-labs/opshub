...
Phase 10 実装状況: **本文取り込み + provenance タグの拡張** ([ADR-0020](adr/0020-full-local-content-retention.md))。`SourceObserved` event に `body` / `provenance_origin` / `provenance_trust` の 3 フィールドが追加され、Slack / Box / GitHub / MS365 の各 connector mapper が要約だけでなく本文も取り込むようになった。連動して `sources` projection の `body` 列 (migration 0018) と FTS5 仮想テーブル `sources_fts` (migration 0019) が追加される。epic #470 / issue #481 (2026-06-07) で `body` は `NOT NULL` に格上げ (ADR-0020 §(d')、migration 0030)。`box_drive` の stat-only path や Google Workspace `google_workspace_file` catch-all 等 metadata-only な connector は ADR-0010 §不変条件 metadata-only rule に従い `body = summary` を emit し、ADR-0019 §不変条件 (b) (FS scan は `open()` しない) を破らずに NOT NULL 制約を満たす。`summary` と `body` の semantic 区別 (前者は preview、後者は full text / search 対象) は維持され、stat-only path では両者が同値、Office 抽出 / Slack message / Outlook 等の本文抽出 path では別物。**取り込み除外設定** (`~/.config/opshub/excludes.yaml`) は channel / sender / repo / path で外部入力を弾き、保存時暗号化は SQLCipher で DB 丸ごと AES-256 (ADR-0021)。詳細は §8.1 を参照。

11 connector の一覧 (Phase 11 で Teams / OneDrive Drive を、Phase 13 で Google Workspace を、Phase 14 で Gmail / Google Calendar を、Phase 21 で Web を追加。Phase 14 で Google 3 connector は `connectors/google_auth/` shared auth foundation = 1 Google account principal + 3-scope 固定 list `drive.readonly + gmail.readonly + calendar.readonly` を共有。Phase 21 の `web` connector は Playwright browser read 層 ([ADR-0037](adr/0037-browser-read-layer-playwright.md)) を駆動し、delta API を持たないため `SourceObserved.fingerprint` 変更検知 (ADR-0019 §決定 (d) pattern) で差分判定する):
...
| Slack | Web API (slack-sdk)。conversation 一覧は `opshub slack conversations` (Phase 17 ADR-0031 で `opshub connector slack conversations` から rename、`users.conversations` 経由、joined-only default + DM/MPIM 統合、default `--format=toml` で `[connectors.slack] channels` 用 snippet 出力 (Phase 19-D ADR-0035 §(a))、`--format=table` で pre-19-D rendering、`--all` で `conversations.list` workspace-wide fallback、type 別固定ソート (`public → private → mpim → im`) + `--since 7d` / `--since 2026-05-01` で activity フィルター。Phase 19-B ([ADR-0034](adr/0034-slack-engagement-axis.md)) で engagement 軸 (= 自分が発言した channel) を導入し Phase 19-D ([ADR-0035](adr/0035-slack-sort-axis-consolidation.md)) で `--activity={mine\|any}` flag を `--sort=name\|last_self_post\|last_activity` に統合。default `--sort=name`(display_name 昇順、ADR-0035 §(b))。Phase 23-G ([#537](https://github.com/ozzy-labs/opshub/issues/537)) で軸を直交化:`--sort=last_self_post` で engagement 軸 (1 度の `search.messages`呼出で per-channel max self-post ts を index 化、`last_self_post_ts`populate、`LAST_POST`列、`search:read`User Token 必須、Bot Token は ConfigError 拒否)。`--sort=last_activity`で any-author 軸 (`conversations.history`per-row、`last_activity_ts`、`*:history`scope)。`--since` は activity `--sort` との併用必須で `--sort=name + --since`は ConfigError 拒否 (旧暗黙 engagement default を撤去、ADR-0035 §(d) 反転)。`--sort=last_self_post\|last_activity` + `--since` なしで暗黙 `--since 90d`cutoff (出力に刻印 + stderr notice、ADR-0035 §(e))。`--all`+`--sort=last_self_post`は非両立 (engagement 軸は self-member channel しか index しないため)、`--sort=last_activity` のみ workspace-wide 可。Phase 14.x #341 / #366) で取得 | `slack_message` | User Token (`xoxp-`) / Bot Token (`xoxb-`) | `[connectors-slack]` |
...
| Web (Phase 21) | Playwright browser read 層 (`src/opshub/browser/core.py`、headless Chromium、専用 user-data-dir、render 後の `page.inner_text("body")` を抽出 + 500K char cap、ADR-0037)。`[connectors.web] pages` に operator が明示登録した URL のみ取得 (**crawler 非該当** = リンク追跡 / sitemap なし、ADR-0010 §Phase 21 改訂 (n))。delta API なしのため抽出後本文の SHA-256 fingerprint で変更検知 (ADR-0019 §決定 (d) pattern、ADR-0010 §Phase 21 改訂 (o))。`opshub web sync` (noun-first ADR-0031) | `web_page` | なし (public ページのみ。認証付きページの `connect_over_cdp` escape hatch は後続 Phase 予約) | `[browser]` (playwright) |

長時間 `opshub <connector> sync` (Phase 17 ADR-0031 で `opshub connector sync <name>` から per-noun group に再編) 中の進捗表示は CLI driver 側で `_ProgressSourceProxy` (`src/opshub/cli/_connector_common.py`) が source service を透過プロキシでラップし、`observe()` 成功ごとに `advance(1)` する形で実現する。`Connector` Protocol および 11 connector 本体は無改修で、進捗表示は connector 層の不変条件 (差分 fetch / cursor 保存 / event 発行依頼) を一切変えない。詳細は [ADR-0026](adr/0026-cli-progress-reporting.md) を参照。
...
Phase 18-B ([ADR-0033](adr/0033-slack-mention-demand-digest.md)) で `slack_demand_digest` projection を追加。Slack の `SourceObserved` event を消費し、`(team_id, channel_id, demand_kind)` 単位で **自分宛 mention / DM の最終 demand timestamp + excerpt** を materialise する。新 connector / fetcher / mapper / event は追加せず純粋な下流追加 (ADR-0002 整合)。物理 schema は migration `0029_create_slack_demand_digest` + `0033_rekey_slack_demand_digest_team_id` (Phase 24-D で natural key を `team_id` 込みに widen)、登録は `projections/registry.py:all_projections()` で `SourcesProjection` の後。Self user id は Phase 24-D ([ADR-0041 §(g)](adr/0041-slack-multi-workspace.md)) で **per-workspace** 解決に拡張した: projection は `{team_id: self_user_id}` map を保持し、各 event の `external_id` (`f"{team_id}:{channel_id}:{ts}"`) から `team_id` を取り、per-alias の token (`SlackAuth(alias).test_token()`) または team-qualified な env override (`OPSHUB_SLACK_SELF_USER_ID__<ALIAS>=T...:U...`) で self id を引く (`U...` id は workspace ごとに別物のため install-wide 単一 id では他 workspace の mention を全 miss する)。CLI debug 表面は `opshub slack mentions list --types ... --demand-kind ... --format table|json|md`。skill 出口は Phase 18-C で MCP `slack.demand.list` tool として追加予定 ([ADR-0033 §決定 (c)](adr/0033-slack-mention-demand-digest.md))。

Phase 19-B ([ADR-0034](adr/0034-slack-engagement-axis.md)) で discovery コマンド `opshub slack conversations --since <when>` に engagement 軸 (= 自分が発言した channel) を導入し、Phase 19-D ([ADR-0035](adr/0035-slack-sort-axis-consolidation.md)) で CLI surface を `--sort=name|last_self_post|last_activity` に統合した。1 度の `search.messages` 呼出で per-channel max self-post ts を index 化し、listing 行を index で filter する。engagement 軸 (`--sort=last_self_post`; ADR-0035 §(c)、Phase 23-G [#537](https://github.com/ozzy-labs/opshub/issues/537) で `--sort` 明示のみに直交化し旧 `--sort=name + --since` 暗黙 default を撤去) と any-author 軸 (`--sort=last_activity`、legacy [#374](https://github.com/ozzy-labs/opshub/issues/374)) は **orthogonal** で、ADR-0033 demand digest 軸 (mention / DM 受信) とも独立。1 つの row は最大 1 つの軸 ts (`last_self_post_ts` or `last_activity_ts`) を carry し、両軸が同時に populate されることはない (ADR-0034 §(g) / ADR-0035 §(c))。`search:read` (User Token only) が engagement 軸の scope 要件として加わり、Bot Token は engagement 軸を満たせないため explicit ConfigError で `--sort=last_activity` への切替を誘導する。projection / event store は touch せず、CLI surface と connector helper のみの変更で完結する (`SlackConversation` dataclass / `list_conversations(sort=...)` 引数 / `_fetch_self_post_index` helper / `_to_connector_failed_search` error mapper / `_emit_implicit_cutoff_notice`)。
...
- **Config 読込**: `config.toml` は起動時に毎回読まれ、優先順位は **`init args > env > toml > defaults`** ([ADR-0032](adr/0032-runtime-toml-config-loading.md))。`OPSHUB_*` 環境変数 (CI / headless 用) は TOML より優先、`OPSHUB_CONFIG_DIR` で config ディレクトリの位置を上書き可能。TOML 不存在は fail せず defaults にフォールバック、不正値は起動時 `ConfigError` で fail-fast (operator 視点の詳細は [docs/troubleshooting.md §3.10](troubleshooting.md))
...
Phase 20 ([ADR-0030](adr/0030-slack-thread-reply-ingestion.md) revised + landed) で `opshub slack sync` を thread reply (late reply 含む) も含む message 単位の全量取得に拡張した。1 回の sync は **2 phase 構成** を順に流す: Phase 1 で `conversations.history` 経由の親 + `reply_count > 0` の親について `conversations.replies` を即時 fetch (`messages[0]` = 親自身は `external_id = f"{team_id}:{channel_id}:{ts}"` UNIQUE 制約により idempotent に弾かれる; Phase 24-B で `team_id` prefix 込みの 3-token に re-key、ADR-0041 §(a))、Phase 2 で 2 軸 compound cursor の `threads` 軸を起点に late reply のみ追加 fetch する。`thread_activity_window` (default 30d、`parse_since` 共通経路 + `"all"` で prune 無効化) を超えた `threads` 軸 entry は Phase 2 happy path 完了時に prune される (mid-iteration crash は resume-safe で entry を残す)。窓経過後の cold thread に投稿された late reply は本経路では追従しない意図された limitation で、必要時は `opshub slack cursor reset --channel <id>` で当該 channel cursor をリセットする (`opshub projections rebuild` は cursor をリセットしない、ADR-0038 §Context)。

Phase 20-B で `connector_cursors.cursor_value` JSON 値を旧 flat-dict `{channel_id: ts}` から **2 軸 compound envelope** `{"channels": {channel_id: 最大 ts}, "threads": {f"{channel_id}:{parent_ts}": 最大 reply ts}}` に張り替えた。両軸とも `_max_ts(prior, new)` で monotonic に advance し、片軸の advance が他軸を巻き戻さない。compound envelope の dump は axes を sorted で deterministic 出力する。旧 flat-dict は silent migration を持たず `ConfigError` で `opshub slack cursor reset --all` を案内する (Phase 23-A [#531](https://github.com/ozzy-labs/opshub/issues/531) で `opshub projections rebuild` 誘導を是正; rebuild は flat-dict event payload を replay して同形に戻すため dead-end だった。`reset --all` は cursor を parse せず空 compound で hard-drop し、空 compound の `ConnectorSyncCompleted` を記録するため rebuild 後も compound schema を保つ。pre-userbase posture、ADR-0034 系列を継承)。DB schema (Alembic migration) は touch せず、既存の `cursor_value` TEXT column の JSON 値のみ拡張する変更で済む。

Phase 24 ([ADR-0041](adr/0041-slack-multi-workspace.md)) で **1 install = N Slack workspace** を first-class 化した。compound cursor は 4 軸 shape (`channels` / `backfill` / `threads` / `team_id`、Phase 22 / 23-H で拡張) ごと **per-alias nest** `{"workspaces": {"<alias>": {...4軸...}}}` の内側に移る (軸の意味・lifecycle は不変)。connector instance は `name = "slack"` 単一のまま (multi-instance 化は不採用)、workspace 分離は `external_id` の `team_id` prefix で行う。workspace は operator が付ける **alias** (操作層 = config table / `--workspace` / keyring slot `connector:slack:<alias>:token`) と Slack `team_id` (データ層 = `external_id` の immutable prefix) の二層 identity で識別する。`sync` は configured workspace を直列 loop し **per-workspace error isolation** (各 workspace 完了ごとに cursor を checkpoint、1 workspace 失敗が他の進捗を巻き込まない)。各 alias の cursor entry は初回 sync で `team_id` を bind し (per-alias bind guard、ADR-0039 の install-wide guard を一般化)、2 alias が同一 `team_id` に解決すると `ConfigError`。旧 23-H top-level shape は silent migration せず `ConfigError` で `opshub slack cursor reset --all` を案内する (posture 継承)。
...
| write | `person.merge` | **Phase 25-D 新規** — 2 person を merge (ADR-0043 §決定 (c) HITL、`WriteCategory.PERSON_MERGE`、`open_world=false`、過剰 merge は `person.split` で取消) |
| write | `person.split` | **Phase 25-D 新規** — 1 identity を新 person に detach (`WriteCategory.PERSON_SPLIT`、`open_world=false`、resolver は split しない operator 専用) |
...
### 2.11b Assistant Agent Layer (Phase 10、形A)
...
1. **MCP tool 面** (§2.11a)
2. **アシスタント 14 Skill** (Phase 12 で 5 → 14 拡張、`docs/skills/<name>/SKILL.md` を opshub SSOT として保持。配信経路は Phase 16-A ([ADR-0029](adr/0029-distribute-assistant-skills-via-opshub-package.md)) で **opshub package 同梱 + `opshub skills install`** に確定。SSOT 位置は [ADR-0004 §決定 (c)](adr/0004-agent-runtime-boundary.md) で変更なし)。Skill catalog SSOT は [docs/assistant-agent.md](assistant-agent.md) (ADR-0004 §決定 (c-2))。
   ...
   **MCP tool 依存マップ** (skill × MCP tool マトリクス、`docs/assistant-agent.md` §MCP tool 依存マップ で詳細表を保持):
...
詳細は [docs/assistant-agent.md](assistant-agent.md) を参照。
...
| 不変条件 | 値 | 振る舞い |
|---|---|---|
| ファイル size 上限 | 50 MB (`max_file_bytes`、`opshub.toml [office] max_file_size_mb` で override) | 超過時 `ExtractResult.body=None` (内部) + `skip_reason="file too large"` + `structlog.warning`。mapper では `body = summary` に fallback して `sources.body` には非空文字列が persist される (epic #470 で `sources.body NOT NULL` 化、ADR-0020 §(d')) |
| 抽出後 text 上限 | 500 000 chars (`max_chars`、`opshub.toml` 上書き可) | 超過時 head-truncate + `\n\n[truncated: original=<N> chars, limit=<M>]` 注記 |
| 抽出失敗 | markitdown exception (corrupted / password-protected / unsupported) | `ExtractResult.body=None` (内部) + `skip_reason="extraction failed: <class>"` + sanitised warning。mapper では `body = summary` に fallback して projection は非空文字列 persist。SourceObserved は metadata のみで発行継続 (epic #470 で `sources.body` 必須化) |
...
### 2.11d Browser Read Layer (Phase 21)

[ADR-0037](adr/0037-browser-read-layer-playwright.md) で **Playwright ベースのブラウザ読み取り層**を新設した。API / ローカル FS scan ベースの既存 11 connector では取れない、ブラウザレンダリングを要する Web ページ (JS 描画 SPA / 動的ページ) の本文を Operational Memory へ取り込む。本層は `src/opshub/browser/core.py` に閉じ、共有インフラとして Phase 21-C `web` connector と Phase 21-D MCP `browser.fetch` tool の両方が駆動する。

**ライブラリ = Playwright 1 本** ([ADR-0037](adr/0037-browser-read-layer-playwright.md) §決定 (a)/(b))。生 CDP (Chrome DevTools Protocol) の独立実装は持たず、低レベルアクセスが要る場合も Playwright の `CDPSession` 経由 (`new_cdp_session` / `connect_over_cdp`) に一本化する (raw WebSocket 直叩き禁止)。Chromium のみ + headless default + opshub 専用 user-data-dir (`core/platform.py` の data dir 配下、ユーザーの普段使い Chrome profile には触れない、§決定 (c))。Chromium binary は `playwright install chromium` が operator 手順で、binary 不在時は `ConfigError` で install コマンドを誘導する (§決定 (g))。

**本文抽出** ([ADR-0037](adr/0037-browser-read-layer-playwright.md) §決定 (d))。`fetch_page` が `page.goto(url, wait_until="load", timeout=...)` → `page.title()` + `page.inner_text("body")` (render 後の DOM 可視テキスト) → 500K char cap (ADR-0025 §決定 (b) と同規律、`opshub.core.text_limits.truncate_with_marker` を共用) の順で組み立てる。timeout / navigation error は `BrowserFetchError` (= `ConnectorFailedError` サブクラス) に変換し、`web` connector の既存 fail-safe 経路が per-page スキップとして扱う。

**設定** (`[browser]` / `OPSHUB_BROWSER__*`、`BrowserSettings`)。`headless` (default `true`) / `channel` (branded build、default `None` = bundled Chromium) / `timeout` (ms、default 30000) / `cdp_endpoint` (`connect_over_cdp` escape hatch、default `None` = launch path)。`ConnectorSettings` 配下ではなく settings root に置くのは、browser 層が connector と MCP の両方から駆動される共有インフラのため。

**sync/async 境界** ([ADR-0037](adr/0037-browser-read-layer-playwright.md) §決定 (h))。browser core は Playwright sync API (CLI-first、opshub の sync SQLAlchemy codebase と整合)。Playwright sync API は asyncio loop 内で直接呼ぶと raise するため、MCP の async handler (`browser.fetch`) は `asyncio.to_thread` 経由で browser core を呼ぶ。

**操作系 (click / fill / submit) は scope 外** ([ADR-0037](adr/0037-browser-read-layer-playwright.md) §決定 (f))。read 層は操作系 API (`page.click` / `page.fill` / `page.set_input_files` 等) を呼ぶ code path を持たないことで構造的に「操作経路の不在」を保証する (§禁止事項 7 write-back ban の web への自然延長)。HITL 承認単位の設計 / Web ページ由来 prompt injection 対策 / ADR-0010 §禁止事項 7 との整理という 3 前提が揃う後続 Phase で解禁する。
...
| `slack_demand_digest` | projection | Phase 18-B (✅ 実装済、migration 0029 → 0032 → 0033、ADR-0033 / ADR-0041) | Slack 自分宛 demand (@mention / DM) の最終発火 timestamp + excerpt。natural key `(team_id, channel_id, demand_kind)` (Phase 24-D [ADR-0041](adr/0041-slack-multi-workspace.md) §(g) で `team_id` を加えて widen、migration `0033_rekey_slack_demand_digest_team_id`。workspace 跨ぎで衝突する channel id を team_id で名前空間分離し self-id も per-workspace 解決)、`demand_kind ∈ {"mention","dm"}` (Phase 23-D migration `0032_drop_mpim_demand_kind` で dead だった `mpim` を撤去、mpim channel は body の `<@self>` 経路で `mention` として検知)、`channel_type ∈ {"im","mpim","private","public"}`、`last_source_id` FK → `sources.id` (provenance、ondelete SET NULL)、`ix_slack_demand_digest_last_demand_ts DESC` + `ix_slack_demand_digest_type_ts` 2 INDEX。replay-order-independent (`last_demand_ts < excluded.last_demand_ts` guard) |
...
Phase 10 で本文をローカルに保持する方針 ([ADR-0020](adr/0020-full-local-content-retention.md)、ADR-0005 を supersede) に移行した。要約のみを残す旧方針はアシスタントユースケース (横断検索 / 再要約 / 返信下書きの文体再現) で上流再取得を強要し、SaaS 側で本文が消えた瞬間に文脈を失う ＝ §1 Local-first と矛盾していた。
...
| 8 | Knowledge graph layer (ADR-0017 + `links` projection (migration 0016) + 4 自動抽出経路 + manual link CRUD + `LinkService` traversal + `opshub link` / `opshub graph` CLI + `--expand-graph` integration、✅ 2026-05-17 完了、epic #128。epic #470 で `--expand-graph` opt-in を撤廃し graph 1-hop 拡張を常時実行に統一、ADR-0017 §決定 (f) revised) | connector-side automatic `SourceReferenced` 発行 / graph visualisation web UI (Phase 8.x) / multi-machine sync (Phase 10+) |
...
| 10 | Assistant Agent Platform (ADR-0020 (full local content retention、ADR-0005 supersede) + ADR-0021 (encryption at rest、SQLCipher + keyring) + ADR-0022 (MCP server surface、stdio + policy-as-data + redact + OTel naming) + ADR-0004 改訂 (形A、opshub は MCP + Agent Skills のみ、runtime なし) + ADR-0016 改訂 (`ReplyDraftCandidatePayload`) + ADR-0017 改訂 (`reply_draft_replies_to` / `referenced_in_reply_draft` link types) + ADR-0010 改訂 (write-back 明示禁止) + 本文ベース embedding (migration 0018) + SQLite FTS5 (migration 0019) + `opshub search` CLI + `opshub mcp serve` CLI + アシスタント 5 Skills (Phase 12 H1 で `personal-brief` / `next-actions` / `reply-draft` / `pr-review` / `find-document` に rename 済) + `tools/skill_scan.py`、✅ 2026-05-31 完了、epic #203) | MS Office 深掘り (Teams + Outlook 本文 + Word/Excel/PowerPoint 抽出、ADR-0025 = Phase 11) / 能動性段階 1-4 (cron 委譲 / 記憶キュレーション / 通知 / filewatch、Phase 12+) / multi-machine sync (Phase 12+) |
...
| 12 | Assistant Skills 拡張 (アシスタント Skill レパートリーを 5 → 14 に拡張: 新規 9 = meeting-prep / research / inbox-triage / external-brief / decision-rationale / handoff-draft / announcement-draft / meeting-followup / source-extract + 既存 5 のうち rename 2 = daily-brief → personal-brief / file-lookup → find-document、区分: read 自律 OK 10 + HITL write 4 + pair structure 4 = personal-brief ↔ external-brief / meeting-prep ↔ meeting-followup / inbox-triage ↔ source-extract / draft family) + 4 新 MCP tools (`search` (FTS5、phrase-quoted default、`raw_query` flag は CLI 専用で MCP schema 除外、`ReadCategory.SEARCH`) + `propose.apply` (HITL、idempotent 正規化、`WriteCategory.PROPOSE_APPLY`、`destructive=false` + `idempotent=true`) + 既存 4 read tools の physical column ベース時間フィルタ = `task.list.updated_after/before` (`tasks.updated_at`) / `inbox.list.created_after/before` (`inbox_items.created_at`) / `decision.list.recorded_after/before` (`decisions.recorded_at`) / `source.list.observed_after/before` (`sources.observed_at`)、ISO 8601 半開区間) + 既存 5 SKILL.md を MCP 直接呼びに統一 (CLI fallback 廃止) + `propose.generate` の `mode` 引数追加 (`inbox_triage` / `source_extract` / `meeting_followup`、persist 経路を持つ 4 mode のみ) + ADR 改訂 3本 (ADR-0004 改訂 (Skills SSOT を opshub `docs/skills/` に移管 + §決定 (c-2) Skill catalog SSOT = `docs/assistant-agent.md` 独立条文化) + ADR-0022 改訂 (§決定 (f) 4 新 MCP tools 契約化) + ADR-0016 改訂 (§決定 (l) draft 系統一方針: persist 境界 = 返信元 source の有無 / `mode` 引数射程 / triage = reply_draft 専用 / Candidate union freeze)) + `docs/assistant-agent.md` を 14 skills 責務マップ SSOT に拡張 (§形A 責務分担 / §アシスタントへの依頼例 / §Skill catalog (read 10 / HITL write 4) / §Pair structure / §HITL boundary / §MCP tool 依存マップ / §できること・できないこと / §セットアップ / §skill security / §関連)、✅ 2026-05-31 完了、epic #253) | アシスタント skill 配信機構 (`ozzy-labs/skills` Renovate preset 経路は Phase 13 / 14 / 15 連続で defer、Phase 16-A [ADR-0029](adr/0029-distribute-assistant-skills-via-opshub-package.md) で opshub package 同梱経路に切り替え確定) / 削除候補 skills (agenda-builder / retrospective / weekly-plan / options-compare / risk-assessment、需要顕在化時に個別追加) / draft 系 persist 需要 (handoff-draft / announcement-draft が persist 要求時に ADR-0016 §決定 (f) versioning パターンで対応) / Phase 16+ = 統合・検索融合レイヤ / 能動性 / 外部書き戻し / Codex / Gemini / Copilot Skills 横展開 |
| 13 | Google Workspace コネクタ (Phase 11 で確立した markitdown 経路を Web API 経由で再利用、Google Docs / Slides / Sheets を Drive API v3 + OAuth Refresh Token + Workspace export → MS Office mediatype → markitdown で取り込む。改訂 3 本: ADR-0010 改訂 (§Phase 13 改訂 (e)-(h) = `google_workspace` 新コネクタ + Drive `files.watch` 禁止 + Workspace export 経路の本文抽出契約 + Drive `changes.list` cursor + TTL 失効時 full-pass fallback 義務 + Refresh Token principal = MS365 / Box pattern、Teams pattern とは別系統である旨を明文化) + ADR-0014 改訂 (§Phase 7 Validation rotation pin リスト 3 件目に `connector:google_workspace:refresh_token` 追加 + env override `OPSHUB_CONNECTOR_GOOGLE_WORKSPACE_REFRESH_TOKEN`) + ADR-0025 改訂 (§決定 (d') 新 source_type 3 種 `google_doc` / `google_slides` / `google_sheets` + §決定 (j) Workspace export 経路 = Drive API `files.export` → MS Office mediatype → markitdown 統一、`extract_workspace_export(bytes, source_type)` で API 表面拡張)。`connectors/google_workspace/` 5 module 構成 (`auth.py` paste-code OAuth + refresh token rotation 書き戻し / `client.py` Drive API v3 + httpx + rate-limit retry / `cursor.py` page token + TTL fallback / `mapper.py` Google mimeType → source_type 分岐 + provenance / `connector.py` content_extraction opt-in wiring + files.export 経由 markitdown 抽出) + `[connectors-google-workspace]` extras (httpx)。✅ 2026-05-31 完了、epic #274 | 画像 OCR (PPT 内画像 + Office 図表、tesseract / pytesseract、Phase 11 OQ7 / Phase 12 §9 / Phase 13 から繰り越し、Phase 14 を経て再 defer = Phase 16+) / 外部書き戻し (Drive write / Teams reply send、新 ADR 要、Phase 16+ 能動性段階で再評価) / 追加コネクタ (Notion / Jira / Linear / Confluence、Phase 13 から繰り越し = Phase 16+) / Google Workspace multi-account 対応 (OQ11、Phase 16+) / Drive `files.watch` + Gmail `users.watch` + Calendar `events.watch` push notification (Phase 16+ 能動性段階で grouping して再評価) / Drive Comments / Suggestions 取り込み (Phase 16+ へ移送、Phase 14 では着手せず) / multi-machine sync (Phase 16+) |
| 15 | Search 品質改善 (FTS5 日本語 tokenizer trigram 化 + 短クエリ LIKE fallback、ADR-0028 新規 + 改訂ゼロ。migration `0028_rebuild_sources_fts_trigram` で `sources_fts` を `unicode61 remove_diacritics 2` から FTS5 built-in `trigram` に物理張り替え + `sources.body` から back-fill + trigger 3 本再作成 (downgrade で元 tokenizer に復元)。`src/opshub/services/search_service.py` に `_MIN_FTS_QUERY_CHARS = 3` 閾値 + `_search_like_fallback` を追加し、1-2 文字短クエリは `LOWER(body) LIKE LOWER(?)` full scan に routing (NFC 正規化 + ASCII case-insensitive + LIKE wildcard escape + `raw_query=True` bypass)。cross-cutting fix = `tests/unit/mcp/test_phase12_handlers.py::_bootstrap_fts_index()` の seed tokenizer も `trigram` に同期 (production と MCP unit boundary 再整合)。`opshub search --help` の `--raw` 説明更新 + `docs/troubleshooting.md` §3.6 日本語 search 節追加。MCP `search` tool (ADR-0022) は契約不変でアシスタント 14 Skill (`find-document` / `research` / etc.) も透過的に恩恵を受ける。✅ 2026-06-02 完了、epic #338 | 形態素 tokenizer 採用 (Lindera / SudachiPy / MeCab、Phase 15 で defer、trigram で operator 体験が不足する場合に ADR-0028 改訂 + tokenizer 評価) / dual index (unicode61 + trigram、Phase 15 で却下、BM25 ranking 劣化観測時に再評価) / `opshub search rebuild-index` 専用 CLI (projection rebuild 経由で十分か、需要顕在化時に切る) / MCP `search` tool 契約改訂 (`raw_query` operator 露出) / 検索結果の semantic re-rank (vector との hybrid score、別 ADR) / snippet() ハイライト / 検索クエリの NFKC 正規化 / 全角半角統一 (operator 観測されてから別 issue) は Phase 16+ |
| 16 | Assistant Skill Distribution via opshub Package Bundling (配信経路改訂 + CLI 実装 + `opshub init` 連携 + in-repo dogfood、新規 ADR 1 本 + 既存 ADR §決定改訂 1 本)。**Phase 16-A** ([`#382`](https://github.com/ozzy-labs/opshub/issues/382)): ADR-0029 (Distribute Assistant Skills via opshub Package Bundling、新規) で配信経路を `ozzy-labs/skills` Renovate preset から opshub Python package 同梱 (`[tool.hatch.build.force-include]` で `docs/skills` → `src/opshub/_skills`) + `opshub skills install` CLI に切り替え。7 軸決定 (配布方針 / 配置パターン / install トリガー / ホスト範囲 / scope default = user / 衝突戦略 default 上書き / ADR 体裁) + scope 境界 (アシスタント 14 = opshub 同梱、ecosystem 共通 = `ozzy-labs/skills` preset 経路、名前空間 disjoint 不変条件) + dogfood 採用判断 (in-repo `.claude/skills/<assistant>/` populate を Phase 16-D で実行) + 非対話 default = install の合理化 (`uv tool install` ユーザー = non-interactive path = actual user path) + ADR-0011 (ecosystem adoption) からの scope carve-out 明文化。ADR-0004 §決定 (c) 改訂 (revision date 2026-06-02 追加 + 配信経路を ADR-0029 にリンク委譲、SSOT 位置 = opshub `docs/skills/<name>/SKILL.md` は不変)。doc 一括更新 = architecture.md / assistant-agent.md / README.md / README.ja.md / mcp-setup.md / upgrading.md / CLAUDE.md / AGENTS.md の "cp -r" 手動 install 手順を pointer 化 (historical 手順は phase-12-plan.md に保持)。`tests/unit/skills/test_core_boundary.py` docstring 整合 + 新 assertion `test_skills_payload_contains_no_python` (`_skills/` 不在時は skip = Phase 16-B 着地前は skip 動作)。**Phase 16-B** ([`#383`](https://github.com/ozzy-labs/opshub/issues/383)): `opshub skills install` / `opshub skills list` CLI 着地 (`--scope {user,project}` / `--host {claude-code,codex,copilot,all}` / `--skip-existing` / `--dry-run` / `--print-paths`)。**Phase 16-C** ([`#384`](https://github.com/ozzy-labs/opshub/issues/384)): `opshub init` 連携 (TTY 時 `rich.prompt.Confirm` 確認 default = yes、非対話は default install、`--install-skills` / `--no-install-skills` で明示上書き)、`install_command` は `opshub.cli.skills` から `opshub.cli.init` が lazy import で呼ぶ。**Phase 16-D** ([`#385`](https://github.com/ozzy-labs/opshub/issues/385)): in-repo dogfood = `.claude/skills/<assistant>/` + `.agents/skills/<assistant>/` populate (project scope、14 件分 × 2 mirror = 28 dir) + `skills-sync-check` pre-commit lefthook hook で SSOT (`docs/skills/`) と mirror を `diff -rq` 比較 (`opshub skills` CLI 経由ではなく直接比較で uv invocation を回避し hook を fast に保つ)。✅ 2026-06-02 完了、epic [`#381`](https://github.com/ozzy-labs/opshub/issues/381) | 統合・検索融合レイヤ (Phase 16+) / 能動性 (Phase 16+) / 外部書き戻し (Phase 16+ + 新 ADR 要) / Codex / Gemini / Copilot Skills 横展開 (Gemini は AGENTS.md 経路、Codex / Copilot は `~/.agents/skills/` 共有経路で着地済) |
