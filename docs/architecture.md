...
| Slack | Web API (slack-sdk)。conversation 一覧は `opshub slack conversations` (Phase 17 ADR-0031 で `opshub connector slack conversations` から rename、`users.conversations` 経由、joined-only default + DM/MPIM 統合、`--format toml` で `[connectors.slack] channels` 用 snippet 出力、`--all` で `conversations.list` workspace-wide fallback、type 別固定ソート (`public → private → mpim → im`) + `--since 7d` / `--since 2026-05-01` で activity フィルター。Phase 19-B ([ADR-0034](adr/0034-slack-engagement-axis.md)) で `--since` の default が engagement 軸 (= 自分が発言した channel) に切替わり、1 度の `search.messages` 呼出で per-channel max self-post ts を index 化、`last_self_post_ts` を populate、`LAST_POST` 列で render する。`search:read` (User Token only) が必要で Bot Token は ConfigError で拒否する。`--activity=any` で旧 #374 挙動 (`conversations.history` per-row、`last_activity_ts`、`*:history` scope) に戻せる。`--all + --activity=mine + --since` は非両立 (engagement 軸は self-member channel しか index しないため)。Phase 14.x #341 / #366) で取得 | `slack_message` | User Token (`xoxp-`) / Bot Token (`xoxb-`) | `[connectors-slack]` |
...
Phase 19-B ([ADR-0034](adr/0034-slack-engagement-axis.md)) で discovery コマンド `opshub slack conversations --since <when>` の default が「自分が発言した channel」(engagement 軸) に切替わった。1 度の `search.messages` 呼出で per-channel max self-post ts を index 化し、listing 行を index で filter する。engagement 軸 (`--activity=mine`、default) と any-author 軸 (`--activity=any`、legacy [#374](https://github.com/ozzy-labs/opshub/issues/374)) は **orthogonal** で、ADR-0033 demand digest 軸 (mention / DM 受信) とも独立。1 つの row は最大 1 つの軸 ts (`last_self_post_ts` or `last_activity_ts`) を carry し、両軸が同時に populate されることはない (ADR-0034 §(g))。`search:read` (User Token only) が新 scope 要件として加わり、Bot Token は engagement 軸を満たせないため explicit ConfigError で誘導する。projection / event store は touch せず、CLI surface と connector helper のみの変更で完結する (`SlackConversation` dataclass / `list_conversations(activity=...)` 引数 / `_fetch_self_post_index` helper / `_to_connector_failed_search` error mapper)。
