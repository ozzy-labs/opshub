...
8 connector の一覧 (Phase 11 で Teams / OneDrive Drive を、Phase 13 で Google Workspace を追加):
...
| Google Workspace | Drive API v3 `changes.list` cursor + `files.export` で Workspace native (Docs / Slides / Sheets) → MS Office mediatype → markitdown (Phase 13 ADR-0010 改訂 (e)-(h)、ADR-0025 §決定 (d') + (j)) | `google_doc` / `google_slides` / `google_sheets` / `google_workspace_file` (catch-all) | OAuth paste-code (Refresh Token + offline access + 自前 refresh + rotation 書き戻し = MS365 / Box pattern、Teams pattern とは別系統) | `[connectors-google-workspace]` (httpx) + `[office]` (extraction 利用時) |
...
1. **MCP tool 面** (§2.11a)
2. **秘書 14 Skill** (Phase 12 で 5 → 14 拡張、`docs/skills/<name>/SKILL.md` を opshub SSOT として保持、配信機構 = `ozzy-labs/skills` CI + Renovate preset は Phase 14+ に defer、ADR-0004 §決定 (c) backout)。Skill catalog SSOT は [docs/secretary-agent.md](secretary-agent.md) (ADR-0004 §決定 (c-2))。
