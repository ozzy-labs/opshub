...
| # | Commit | 概要 | 想定 PR # |
|---|---|---|---|
| 5 | `feat(core): foundational utilities` | `core/{ids,time,logging,errors,config}.py`。ULID 生成、tz-aware datetime helpers、structlog config、`OpsHubError` 基底、Pydantic Settings 基底。後続 step が依存する foundation | PR #3 |
| 6 | `feat(db): set up sqlalchemy engine and alembic` | `db/{engine,unit_of_work,schema}.py` + alembic init (`src/opshub/db/migrations/`)。`~/.local/share/opshub/db/opshub.sqlite` デフォルトパス解決 | PR #4 |
| 7 | `feat(db): initial migration with events and embeddings tables` | `0001_events.py` + `0002_embeddings.py` の 2 migration。`embeddings` は Phase 4 まで空運用、`UNIQUE(entity_type, entity_id, model_id, model_version)` 制約 (ADR-0012) | PR #5 |
| 8 | `feat(domain): task events` | `domain/events/{base,task}.py`。`DomainEvent` 抽象 + `TaskCreated` / `TaskActivated` / `TaskCompleted`、Pydantic `frozen=True` + discriminator + `schema_version` (ADR-0002) | PR #6 |
| 9 | `feat(services): task service` | `services/task_service.py`。CLI / agent からの command を検証 → event を append → projector に通知。lock は Phase 2 で実装 (ADR-0004) | PR #7 |
| 10 | `feat(projections): tasks projection + replay test` | `projections/{base,tasks,rebuild}.py`。冪等テスト (`projections rebuild` を 2 回 → 同一結果) を `tests/integration/` に (Principles §8) | PR #8 |
| 11 | `feat(vectors): pluggable embedder/store protocols` | `vectors/{embedder,store}.py` の Protocol 定義のみ (Phase 4 用、実装ゼロ)。interface freeze 用 unit test 併設 (ADR-0012) | PR #9 |
| 12 | `feat(config): settings for storage/workspace/embedding` | Pydantic Settings 拡張。`[storage]` (db path / cache path) + `[workspace]` (workspace root) + `[embedding]` (backend = "disabled" デフォルト) の 3 section (ADR-0012) | PR #10 |
| 13 | `feat(cli): bootstrap commands (init, db migrate)` | `opshub init` (first-time setup: dir 作成、config 初期化、migration 適用)、`opshub db migrate` (apply pending)。Typer lazy import 規約に従う (ADR-0001) | PR #11 |
| 14 | `feat(cli): task commands (create, list)` | `opshub task create` / `task list`。`--format md` 等の出力オプションを Phase 1 で確定 | PR #12 |
| 15 | `feat(cli): ops commands (projections, embeddings)` | `opshub projections rebuild`、`opshub embeddings status` (backend=disabled / 件数 0 を表示) | PR #13 |
| 16 | `feat(markdown): task list rendering to workspace` | `markdown/{render,tasks}.py` + `opshub workspace generate`。Jinja2 template、冪等テスト (Principles §8) | PR #14 |
| 17 | `test: end-to-end lifecycle + docs: readme quickstart` | `tests/integration/test_lifecycle.py` (create → list → generate)、README に Quickstart、cold-start 観測 (`time opshub version`: 上限 300ms / 目標 120ms) | PR #15 |

**論理グルーピング** (milestone 候補):

- **Foundation** (PR #3-#5 / step 5-7): core utilities + db engine + 初回 migration
- **Event Sourcing Core** (PR #6-#8 / step 8-10): domain events + service + projections
- **Interfaces** (PR #9-#10 / step 11-12): Phase 4 用 Protocol + 設定
- **CLI** (PR #11-#13 / step 13-15): 3 種類の CLI command 群
- **Output & Verification** (PR #14-#15 / step 16-17): markdown 生成 + E2E
...
| Milestone | PR | step |
|---|---|---|
| Phase 1: Foundation | PR #3-#5 | 5-7 |
| Phase 1: Event Sourcing Core | PR #6-#8 | 8-10 |
| Phase 1: Interfaces | PR #9-#10 | 11-12 |
| Phase 1: CLI | PR #11-#13 | 13-15 |
| Phase 1: Output & Verification | PR #14-#15 | 16-17 |
