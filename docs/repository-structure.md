...
│   ├── commitment.py               # コミットメント台帳 CLI (scan / list / resolve / dismiss / reopen、scan は LLM 必須 + 進捗、list/状態遷移は LLM 不要、ADR-0042) [P25]
│   ├── _wiring.py                  # 内部 helper: service/projection の組み立て (briefing / proposal / auto-embed hook / link / person / commitment service 含む) [P1+2+5+6+8+25]
...
│   │   ├── commitment.py           # CommitmentScanStarted / Completed / Failed (scan bracket、cursor 軸) + CommitmentExtracted (LLM 抽出、source_ref) + Resolved / Dismissed / Reopened (状態遷移、ADR-0042) [P25]
...
│   ├── commitments/                # CommitmentScanService: 手動スキャン抽出 + 台帳閲覧 + 状態遷移 (ADR-0042) [P25]
│   │   ├── __init__.py             # [P25]
│   │   ├── prompts.py              # 抽出 system/user prompt (delimiter-wrap + html-escape、authorship priming) [P25]
│   │   └── service.py              # scan (LLM call は UoW 外、direction = self-id 信号、counterparty = person:<id>) / list (LLM 不要) / resolve / dismiss / reopen [P25]
...
│   ├── commitments.py              # コミットメント台帳 (CommitmentsProjection / `commitments_table`、source_ref UPSERT + state 遷移、migration 0037、ADR-0042) [P25]
│   └── commitment_scan_cursor.py   # 手動スキャン watermark singleton (CommitmentScanCursorProjection / `commitment_scan_cursor_table`、connector_cursors 同型、migration 0038、ADR-0042) [P25]
├── connectors/                     # [P1+2+3]
