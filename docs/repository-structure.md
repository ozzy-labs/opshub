...
│   └── google_workspace/           # Google Workspace connector (Phase 13 G3 + G4、ADR-0010 §Phase 13 改訂 (e)-(h) + ADR-0014 改訂 + ADR-0025 改訂 (d')+(j)) [P13]
│       ├── __init__.py             # register_connector(GoogleWorkspaceConnector()) side effect [P13]
│       ├── auth.py                 # GoogleWorkspaceAuth: paste-code OAuth (`drive.readonly` scope、`access_type=offline` + `prompt=consent`) + Refresh Token rotation 書き戻し (MS365 / Box pattern、ADR-0010 §Phase 13 改訂 (h)) + keyring slot `connector:google_workspace:refresh_token` (ADR-0014 §Phase 7 Validation 3 件目) + env override `OPSHUB_CONNECTOR_GOOGLE_WORKSPACE_REFRESH_TOKEN` [P13]
