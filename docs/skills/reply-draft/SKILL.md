---
name: reply-draft
description: 「返信案を考えて」「下書き作って」「これに返信したい」と頼まれたら、opshub の propose --kind reply_draft を CLI 経由で叩いて返信下書きを生成する。外部 SaaS への送信は行わず、ユーザーが下書きを確認して手で送る。Sub-issue E で実装済みの ReplyDraftCandidatePayload を利用。
---

# reply-draft — 返信下書きを opshub に生成させる

opshub の `propose generate --kind reply_draft` (CLI、または将来 MCP の `propose.generate` write tool) を経由して返信下書きを作る。Phase 10 Sub-issue D で書き、Sub-issue E (#217 merged) で `ReplyDraftCandidatePayload` が実装済み。ADR-0016 §決定 (i)+(j)+(k) で吸収された。

## 何が起きるか

1. ユーザーが「これに返信下書き作って」「この slack に返信案」「メール返信どうする?」と頼む
2. ホストが本 skill を発火
3. ホストが対象の `source_id` / `source_type` を特定 (recall.search で source を引く or ユーザー入力)
4. CLI 経路で `opshub propose generate --kind reply_draft --reply-to <source_id>:<source_type>` を実行
...
`hits[]` から source_id + source_type を抜き出す。
...
opshub propose generate \
  --kind reply_draft \
  --reply-to <source_id>:<source_type> \
  --expand-graph
```

`--expand-graph` (ADR-0017) は知識グラフから文脈 source を拾うフラグ。ADR-0016 §決定 (k) の `<context_source>` 注入経路。
...
```bash
opshub propose list --kind reply_draft --status pending
```
...
## 返信元
- source_id: ...
- source_type: slack_message / ms365_outlook / github_issue / ...
