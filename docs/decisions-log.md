...
Phase 12 (Assistant Skills 拡張、epic #253) は 2026-05-31 に完了。アシスタント Skill レパートリーを **5 → 14** に拡張し、4 新 MCP tools (`search` FTS5 + `propose.apply` HITL idempotent + 既存 4 read tools の physical column ベース時間フィルタ) を露出、既存 5 SKILL.md を MCP 直接呼びに統一、`propose.generate` の `mode` 引数追加 (`inbox_triage` / `source_extract` / `meeting_followup`) を ADR 改訂 3 本 (ADR-0004 改訂 = §24、ADR-0022 改訂 = §25、ADR-0016 改訂 = §26) で吸収した。新規 9 skills = meeting-prep / research / inbox-triage / external-brief / decision-rationale / handoff-draft / announcement-draft / meeting-followup / source-extract、既存 5 のうち rename 2 = daily-brief → personal-brief / file-lookup → find-document。区分は read 自律 OK 10 + HITL write 4、pair structure 4 = personal-brief ↔ external-brief / meeting-prep ↔ meeting-followup / inbox-triage ↔ source-extract / draft family。Skill catalog (14 skills 責務マップ + HITL boundary + MCP tool 依存マップ + pair structure) の SSOT は `docs/assistant-agent.md` を 10 § 構成に拡張して集約した (§形A 責務分担 / §アシスタントへの依頼例 / §Skill catalog / §Pair structure / §HITL boundary / §MCP tool 依存マップ / §できること・できないこと / §セットアップ / §skill security / §関連)。実装は Wave 構成 (Wave 1 = H1 foundation #262 / Wave 2 = H2 info-gathering #265 / H3 analysis #264 / H4 HITL write #266 / H5 draft #263 を 4 並列 / Wave 3 = H6 closeout #259) + hotfix 2 件 (#267 ruff format + #268 mypy strict) で完了。新 ADR ゼロ、改訂 3 本 (ADR-0004 / 0022 / 0016) に縮退 (Phase 10 の 3 新規 + 4 改訂 → Phase 11 の 1 新規 + 2 改訂 → Phase 12 の 0 新規 + 3 改訂、と縮退継続)。`ozzy-labs/skills` 配布完成 (ADR-0004 §決定 (c) backout) と削除候補 skills (agenda-builder / retrospective / weekly-plan / options-compare / risk-assessment) の需要顕在化時の追加は Phase 13+ に defer。詳細は §24 / §25 / §26 と `docs/phase-12-plan.md` を参照。**Phase 16 後追補 (2026-06-02)**: `ozzy-labs/skills` 配布完成は Phase 13/14/15 連続で defer されたため、Phase 16-A ([ADR-0029](adr/0029-distribute-assistant-skills-via-opshub-package.md) 新規 + ADR-0004 §決定 (c) 再改訂) で配信経路を `ozzy-labs/skills` Renovate preset から opshub Python package 同梱 + `opshub skills install` に切り替えた (SSOT 位置 = opshub `docs/skills/<name>/SKILL.md` は不変)。Phase 16-B/C/D で実装着地済 (`opshub skills install` / `opshub init` 連携 / in-repo dogfood)。
...
| provenance タグなしで本文保持 (緩和層を持たない) | sources に出自・信頼度タグを追加し低信頼外部本文を agent context へ渡す際に明示 | 外部本文をそのまま agent context へ流すと間接プロンプトインジェクションでアシスタントが乗っ取られる、本文保持に転換する以上攻撃面縮小を偶発的 (本文を持たない) から明示的 (出自・信頼度で区別) に設計し直すのは必須 | ADR-0020 |
...
| MCP tool を CLI command 1:1 で機械生成 (低レベル粒度) | 読み取り系は CLI 同等粒度 (recall / search / brief)、アシスタントユースケース粒度は Sub-issue D の Agent Skills で表現する二段構え | CLI は人間が叩く前提で sub-verb / flag が細かく agent の学習負荷が高い、アシスタントユースケース粒度とずれて複数 tool 逐次呼び出しオーバーヘッドが大きい、MCP tool は ①コア operation を直接露出し組み立ては Skills で行う方が Phase 10 形A と整合 | ADR-0022 |
...
| 却下案 | 採用案 | 理由 |
|---|---|---|
| Reply-draft を新 ADR / 新 sub-system (`ReplyDraftService` + 専用 projection + `opshub reply ...` CLI) として独立 | Phase 6 propose lifecycle に `reply_draft` candidate kind を 1 つ追加して吸収 (ADR-0016 §決定 (i)) | Phase 6 で確立した generate→review→apply / triage / HITL / 既存 service 経由 / idempotent key / schema versioning が再利用可能、独立 sub-system は重複と CLI 表面の肥大化、`opshub propose generate --reply-to <source_id>` 1 経路で mental model が小さい (Sub-issue D アシスタント Skill 表とも整合) |
...
| ADR-0004 §決定 (c) Agent Skills 配布 | アシスタント 5 skill (daily-brief / next-actions / reply-draft / pr-review / file-lookup) は opshub 本体に同梱せず、`ozzy-labs/skills` リポに SKILL.md 実体を置き handbook ADR-0016 の `@ozzylabs/skills` Renovate preset 経由で配布。opshub 本体は仕様・catalog (`docs/assistant-agent.md`) と skill security scan ロジック (`tools/skill_scan.py`) のみ保持 |

| 却下案 | 採用案 | 理由 |
|---|---|---|
| アシスタント層の boundary を新 ADR で起票 | ADR-0004 §決定 (a)(b)(c) として改訂吸収 | 既存 boundary 不変条件と同根 (auditability / safety / coordination / replayability)、新 ADR は概念的二重化、改訂で historical context (Phase 1-9 の CLI-first → Phase 10 の MCP 並列追加) が同一 ADR に残る |
...
Phase 12 H1 (2026-05-31) で ADR-0004 を改訂し、Phase 10 改訂時の前提 (`ozzy-labs/skills` preset 配布完成) を意図的に backout した上で、`docs/skills/<name>/SKILL.md` を opshub リポ内 SSOT として正式に認める。配信機構整備 (`ozzy-labs/skills` 側 CI + Renovate preset) は Phase 13+ に defer。同時に新 §決定 (c-2) として **Skill catalog SSOT = `docs/assistant-agent.md`** を独立条文化し、14 skills 体制 (Phase 10 で導入した 5 skill のうち `daily-brief` → `personal-brief` / `file-lookup` → `find-document` に rename された 5 + Phase 12 H2-H5 で追加される 9 = 計 14) の責務マップ / HITL boundary / MCP tool 依存マップ / pair structure を集約。Skill catalog を ADR 化しない判断 = 改訂頻度の高さと ADR の「決定の根拠」性質との相性悪さを優先。**Phase 16-A 後追補 (2026-06-02)**: Phase 13/14/15 連続で `ozzy-labs/skills` 配布完成が defer された結果、ADR-0029 (Distribute Assistant Skills via opshub Package Bundling) を新規追加 + ADR-0004 §決定 (c) を再改訂 (revision date 2026-06-02 追加) し、配信経路を opshub Python package 同梱 + `opshub skills install` に切り替えた。SSOT 位置 (opshub `docs/skills/<name>/SKILL.md`) は本節の決定どおり維持。詳細は §32 を参照。
...
| Skill catalog (14 skills 責務マップ) を新 ADR (ADR-0026 等) として起票 | `docs/assistant-agent.md` を SSOT として集約、ADR 化しない | skill 追加 / rename / description 拡張は Phase 単位で何度も発生、ADR は「決定の根拠」を凍結するドキュメントタイプで頻繁な改訂と相性悪い、横断ビューの一元化として既に `docs/assistant-agent.md` が機能 |
| skill 単位の責務を `docs/assistant-agent.md` 1 ファイルに集約 | 個別 SKILL.md (責務縦) + assistant-agent.md (catalog 横) の 2 階層維持 | 個別 SKILL.md は host 側 skill loader が直接読む対象、catalog 集約だと skill 単位の load contract が成立しない |
...
| 却下案 | 採用案 | 理由 |
|---|---|---|
| Google Workspace 用に新 ADR (ADR-0028 等) で token storage 経路を起票[^adr-0028-disambig] | ADR-0014 §Phase 7 Validation 節への rotation pin リスト追加で吸収 | ADR-0014 の Decision (keyring + 規約 key + env override + extras 隔離) は MS365 / Box / Slack / GitHub で 6 件目の追加でも崩れない (Phase 7 validation で確認済)、新 ADR は概念的二重化 |

[^adr-0028-disambig]: 本表の `ADR-0028 等` は Phase 13 G1 起草時点の **仮称**。実 ADR-0028 は §35 (Phase 15 起票) で `FTS5 sources_fts tokenizer choice` (trigram 化 + 短クエリ LIKE fallback) として消費済。本却下案の起票自体は採用されていないため、番号衝突はない。
...
| 新 keyring slot (`connector:google_mail:refresh_token` / `connector:google_calendar:refresh_token`) を追加 | Phase 13 確定の `connector:google_workspace:refresh_token` 1 slot を 3 connector が共有、新 slot 追加なし | 1 Google account = 1 principal が opshub アシスタント MVP の前提 (ADR-0018 同根拠)、同一 Google account から Drive / Gmail / Calendar を取り込むのが自然な操作モデル、3 つの slot を別管理する意味なし (3 つすべてに同じ refresh token を入れることになり冗長)、scope 拡張で 1 slot 3 connector 共有が成立する非対称性は Google OAuth エコシステムの account 単位 token 設計に由来 |
...
| 却下案 | 採用案 | 理由 |
|---|---|---|
| Gmail / Calendar 用に新 keyring slot を追加 (Phase 13 google_workspace slot とは別管理) | Phase 13 slot を 3 connector で共有、scope のみ拡大 | 1 Google account = 1 principal が opshub アシスタント MVP の前提、3 つの slot を別管理する意味なし (同じ refresh token を 3 つ入れることになり冗長)、scope 拡張で 1 slot 3 connector 共有が成立する非対称性は Google OAuth エコシステム由来 |
...
## 33. Observability & troubleshooting logging closeout (epic #317、ADR-0027、T4、2026-06-01)

CLI 全体に verbosity / debug surface (`-v` / `-q` / `--debug` / `--log-format` / `--log-file` + 対応する `OPSHUB_LOG_LEVEL` / `OPSHUB_LOG_FORMAT` / `OPSHUB_DEBUG` / `OPSHUB_LOG_FILE`) を追加し、ログ全体を redaction processor 経由に構造化した。本作業は [ADR-0027](adr/0027-observability-and-troubleshooting-logging.md) を新規起票して 4 PR (T1-T4) で end-to-end に完了させた。**新 ADR 1 本のみ、既存 ADR の改訂はなし**。

- **Wave 構成 (実績、2026-06-01)**: **T1** = `core/logging.py` 拡張 (redaction processor + `resolve_log_settings` + `format_debug_traceback` + `--log-file` 0600) PR #329。**T2** = root callback の 5 グローバルフラグ配線 (`-v` / `-q` / `--debug` / `--log-format` / `--log-file`、`OPSHUB_DEBUG=1` env export 含む) PR #331。**T3** = connector sync / MCP serve の `--debug` opt-in 経路 (デフォルト = 型名のみ維持、`--debug` 時のみサニタイズ済みメッセージ + traceback を stderr 追加、MCP subprocess は env-only) PR #330。**T4** = docs 一括 (`docs/troubleshooting.md` 新規 + SECURITY.md §Debug-safe logging + README / README.ja / mcp-setup §8.1 / architecture §2.2 / AGENTS / CLAUDE / decisions-log) + `/security-review` epic closeout。T1-T3 は前 wave 完了済から線形に積み上げ、T4 は docs / closeout 専従。
- **R 不変条件 (6 件)**: R1 全ログ値が structlog redaction processor 経由 (`format_exc_info` の後・renderer の前)。R2 `--debug` の full traceback は `format_debug_traceback` でサニタイズ。R3 connector sync 失敗時の `ConnectorSyncFailed.error_message` は例外型名のみ (event log の token surface を広げない、成功時の `synced ...` は stdout のまま)。**stderr** サマリは Phase 23-B ([#532](https://github.com/ozzy-labs/opshub/issues/532)) でサニタイズ済み本文をデフォルトに昇格し `sync failed: <Type>: <sanitised-msg>` を出す (`sync failed:` prefix は grep 互換のため維持)、`--debug` 時はサニタイズ済み traceback を stderr に opt-in 追加。R4 既知トークン形状 (`sk-` / `ghp_` / `github_pat_` / `xox*-` / `AKIA` / `AIza` / JWT / `Bearer`) は全 verbosity で marker 化。R5 `--log-file` 指定時のファイル新規作成は `os.open(O_CREAT|O_WRONLY|O_APPEND, 0o600)`。R6 `opshub --help` cold-start ≤300 ms 維持 (`test_cli_imports.py` の whitelist に `opshub.core.logging` を持ち込まず、`cli/app.py` 内で lazy import)。
- **MCP boundary との関係**: ADR-0022 §(b) の MCP 応答 token passthrough 禁止は `--debug` 相当でも維持 (MCP 応答経路は別レイヤの `mcp/_redact.redact_secrets`、structlog processor とは独立した第二層)。`opshub mcp serve` は subprocess として spawn される性質上 root callback を経由しないため、env-only (`OPSHUB_LOG_LEVEL` / `OPSHUB_DEBUG` / `OPSHUB_LOG_FORMAT` / `OPSHUB_LOG_FILE`) で制御する設計に確定。
- **ADR 番号運用の取り違え訂正**: epic 起票当初は ADR-0026 として議論していたが、epic #316 の close で ADR-0026 = "CLI progress reporting" が確定済だったため、本 epic は **ADR-0027 "Observability & troubleshooting logging"** として独立番号で起票し直した (T1 PR #329 で `docs/adr/0027-observability-and-troubleshooting-logging.md` 公開)。連動 issue 本文 / plan / 関連 PR 本文の ADR-0026 参照はすべて ADR-0027 に書き換え済み。
- **scope 外波及チェック (T4 で実施)**: `src/opshub/cli/app.py` の root callback `help=` テキスト / `src/opshub/core/logging.py` の `_TRUTHY` / `_FALSY` accept-list / `src/opshub/cli/connector.py` の `_is_debug_enabled` / `src/opshub/mcp/server.py:serve_stdio` の env-only resolution を docs から逆引きして spec drift がないことを確認。docs と実装で文字列・enum・優先順位が一致 (Phase 14 audit cluster C 同型ミス防止)。
- **Phase 15+ outlook**: opt-in OTel exporter (現状は structlog OTel GenAI naming のみ、exporter は `mcp-otel` extras で予定) / `--quiet` を rich progress と組み合わせた "ログ抑制 + 進捗だけ表示" モード / アシスタント 14 Skill 側からのトラブルシュート手順 (skill 内エラー時に operator に `--debug` で再現を促す pattern) は Phase 15+ で再評価。本 epic は CLI / MCP のログ verbosity surface に scope を限定して締めた。

## 34. Epic #317 post-merge audit followup (2026-06-02)

Epic #317 closeout (§33) 直後に 4 観点 (対応漏れ / docs 更新漏れ / docs vs 実装乖離 / テスト漏れ) で audit し、HIGH 3 件 / MEDIUM 1 件 / LOW 4 件を 1 PR で一括対応した (`fix/issue-317-post-merge-audit-followup`、`Refs #317`)。

- **H1 — `-vv` が `--debug` を含意する契約を実装に反映**: `docs/troubleshooting.md` §1 と本 §33 の R3 で「`-vv` のとき `OPSHUB_DEBUG=1` が自動 export」と書かれていたが、`src/opshub/cli/app.py:252-253` は `settings.debug=True` でのみ env export し、`src/opshub/core/logging.py:191` は `--debug` のみで `debug=True` を flip していた。`resolve_log_settings` の `resolved_debug` 算出を `debug or env_debug or (cli_level == "DEBUG")` に拡張し、CLI 経路で DEBUG レベルに到達する全パス (`-vv` / `-vvv`...) が一律 `debug=True` を flip するように直した。**挙動意味論変更**: 既存 operator スクリプトで `-vv` を「DEBUG ログだけ欲しい / `--debug` の error wrapper は要らない」目的で使っていた場合、サニタイズ済み full traceback が `stderr` に追加表示される + `OPSHUB_DEBUG=1` が `os.environ` に export されるようになる (子プロセスにも伝搬する)。env 経路 (`OPSHUB_LOG_LEVEL=DEBUG`) は本契約の対象外で、`debug` flag を flip するには `OPSHUB_DEBUG=1` を明示する (後方互換)。ADR-0027 §決定 (a) にこの契約を明文化した。
- **H2 — connector sync 失敗サマリは stderr (docs 訂正)**: `src/opshub/cli/connector.py:346` は `typer.echo(..., err=True)` で stderr に出力しているが、`SECURITY.md:201-207` / `docs/troubleshooting.md:40,48` / connector.py docstring / inline comment が「stdout サマリ」と誤記していた。実装側を SSOT として docs / コメントを stderr に訂正。成功時の `synced <name>: N item(s) observed` は stdout のままなので、「成功 = stdout / 失敗 = stderr」と明確に分けて書き直した (`2>&1` / `2>err.log` のキャプチャ例を inline comment に追記)。
- **H3 — フラグ衝突 integration test 追加**: epic #316 (`--no-progress`) と epic #317 (`-v` / `-q` / `--debug` / `--log-format` / `--log-file`) の同時指定が裸の状態で衝突しうるが、`tests/integration/test_cli_troubleshooting_options.py` には pairwise 組み合わせの test が無かった。`TestFlagCombinations` クラスを追加し `--no-progress --debug` / `--no-progress -vv` / `--no-progress -qq` / `--progress --log-format json` の 4 組合せを pin。`TestVerboseQuietPrecedence` で `-v -q` / `-vv -q` の precedence (quiet 優先) も明文化 (L4 と一緒に対応)。
- **M1 — MCP OTel ログ層の token 形状網羅 (8/8)**: `tests/unit/mcp/test_logging_redaction.py` が `sk-` / `Bearer` / `xoxb` の 3 形状しか pin していなかったので、`ghp_` / `github_pat_` / `AKIA` / `AIza` / JWT の 5 形状を parametrize 経由で追加 (`test_otel_log_redacts_token_shape` 1 関数、5 値)。
- **L1 — `--log-format` 不正値の silent fallback**: `typer.Option` に `click.Choice` 制約が無く `core/logging.py:138-141` で `auto` に silent fallback する挙動を `docs/troubleshooting.md` §1 に 1 文追記。invariant tightening は避け docs 追記のみで対応。
- **L2 — `_FALSY` dead set を unused 削除**: `src/opshub/core/logging.py:73` の `_FALSY = frozenset(...)` は `_coerce_bool_env` (`:117`) で参照されていなかった。docs (`docs/troubleshooting.md` §2) の `OPSHUB_DEBUG` falsy 受理表は「未知の値はすべて false」契約が SSOT (truthy-only match) なので、`_FALSY` を残すと「未知の falsy 値を弾く意図がある」と誤読されるリスクがあるため削除。`_progress.py` 側の `_FALSY` は同モジュール内で `_enabled()` が実際に使っているので touch しない。
- **L4 — `-v -q` precedence の docs 化**: `src/opshub/core/logging.py:160-163` 既存コメントの「`-q` が勝つ」契約を `docs/troubleshooting.md` §1 に 1 文追記 + integration test (`TestVerboseQuietPrecedence`) で pin (H3 と同じ commit でカバー)。
- **L3 — `.env` 値の専用 pin test**: audit 時点で「形状ベース redaction で実害なし」評価のため今回は skip。Phase 15+ の secret-shape 拡張 (新 SaaS 独自形式 token) を入れる際に追加検討する。
- **scope 外波及チェック**: H1 の意味論変更で、`settings.debug` を見る既存呼び出し元 (`src/opshub/cli/app.py:252` env export / `tests/integration/test_cli_troubleshooting_options.py` 既存 assertion / `tests/unit/core/test_logging.py:test_double_v_lifts_to_debug`) を grep で洗い出し、`-vv` で `debug=True` を assert する parametrize を `tests/unit/core/test_logging.py` に追加。H2 で「stdout サマリ」相当の表記を repo 全体 (`src/` / `docs/` / `SECURITY.md` / `decisions-log.md` / `architecture.md`) で grep し、漏れなく stderr に訂正 (`embeddings.py:99` の「stdout summary」は embeddings の成功サマリ = stdout なので touch しない)。MCP 経路で OPSHUB_DEBUG 自動 export が悪影響しないか確認: `opshub mcp serve` subprocess は親環境変数を継承するので、`-vv` 経由で `OPSHUB_DEBUG=1` が立つと subprocess の `serve_stdio` も DEBUG レベル + debug flag を観測する = 意図通り。

## 35. Phase 15 完了 (Search 品質改善、ADR-0028、epic #338、2026-06-02)

Phase 14 closeout (§32、2026-05-31) を受けて、Phase 15+ outlook で forecast されていた「Search 品質改善 (FTS5 日本語 tokenizer 修正)」を Phase 15 として起票し、**2026-06-02 に完了** した (epic #338、`docs/phase-15-plan.md` を SSOT)。Phase 10 で導入した `opshub search` (migration 0019、`sources_fts`) が日本語自然文 (例: 「boxの権限」「進捗記入」「依頼」) で 0 hit になる問題を、`sources_fts` の tokenizer を `unicode61 remove_diacritics 2` から FTS5 built-in `trigram` に張り替え + SearchService 層で 1-2 文字短クエリを `LOWER(body) LIKE LOWER(?)` にルーティングする fallback を入れて根治した。形 A (能動性なし) + 外部書き戻しなし + immutable migration 規範を Phase 10〜14 から継承。

- **採用 ADR**: 新規 1 本 (**ADR-0028 = FTS5 sources_fts tokenizer choice**)、改訂ゼロ。Phase 11 流の「単一 ADR + 単一トピック」路線を継承しつつ、本 Phase は新規 ADR 1 本ルート (Phase 11 = 1 新規 + 2 改訂、Phase 12 = 0 新規 + 3 改訂、Phase 13 = 0 新規 + 3 改訂、Phase 14 = 0 新規 + 2 改訂、**Phase 15 = 1 新規 + 0 改訂**)。形態素 tokenizer (Lindera / SudachiPy / MeCab) や dual index (unicode61 + trigram) は overkill のため不採用 (ADR-0028 §Alternatives で却下記録)。MCP `search` tool 契約 (ADR-0022) は無変更。
- **Wave 構成 (実績、2026-06-02)**: **Wave 1 (S1)** = PR [#346](https://github.com/ozzy-labs/opshub/pull/346) (ADR-0028 起草 + `docs/phase-15-plan.md` + `docs/adr/README.md` 索引 + AGENTS.md Status 行への Phase 15 着手追記、entry PR で実装変更なし)。**Wave 2 (S2)** = PR [#363](https://github.com/ozzy-labs/opshub/pull/363) (`src/opshub/db/migrations/versions/0028_rebuild_sources_fts_trigram.py` 新規 = drop trigger 3 + drop `sources_fts` → create with `tokenize='trigram'` → back-fill from `sources.body` → re-create 3 triggers、downgrade 経路は元 tokenizer に復元、`tests/integration/test_phase15_fts_trigram_migration.py` 14 cases)。**Wave 3 (S3)** = PR [#364](https://github.com/ozzy-labs/opshub/pull/364) (`src/opshub/services/search_service.py` に `_MIN_FTS_QUERY_CHARS = 3` 閾値 + `_search_like_fallback` 追加、NFC 正規化 + ASCII case-insensitive + LIKE wildcard escape + `raw_query=True` bypass、`tests/unit/services/test_search_service.py` +14 tests + `tests/integration/test_phase15_search_japanese.py` e2e + `tests/unit/cli/test_search.py` smoke + cross-cutting fix = `tests/unit/mcp/test_phase12_handlers.py::_bootstrap_fts_index()` の seed tokenizer を `trigram` に同期)。**Wave 4 (S4)** = closeout PR (本 PR、`docs/troubleshooting.md` §3.6 日本語 search 節追加 + AGENTS.md Phase status 行 `Phase 1-15 complete` + `CLAUDE.md` `opshub search` 言及追記 + `opshub search --help` の `--raw` 説明更新 + `docs/phase-15-plan.md` Status header `Phase 15 complete (2026-06-02)` + sub-issue 状態列 ✅ merged 反映 + OQ8-OQ12 確定 + 本節)。
- **OQ8-OQ12 確定 (S3 PR #364 §Open Questions confirmed)**: OQ8 (閾値) = `len(NFC(query).strip()) < 3` → LIKE fallback (3-char は 1 trigram で FTS5 path が native に処理、責務 overlap 回避)。OQ9 (case / NFC) = `LOWER(body) LIKE LOWER(?)` + `unicodedata.normalize("NFC", query)` (trigram default ASCII case-insensitive と整合、Japanese は no-op、composed/decomposed paste を symmetric に hit)。OQ10 (filter / limit 適用順) = `LOWER(body) LIKE LOWER(?) [AND connector_name = ?] AND body IS NOT NULL ORDER BY observed_at DESC LIMIT ?` (full scan 受容、operator-scale (≤100K rows) で sub-second、NULL-body 行 = Phase 3-9 historical + box_drive ADR-0019 を明示除外、dedicated index は Phase 16+)。OQ11 (`--raw` fallback) = bypass (operator MATCH の silent rewrite を回避、ADR-0028 §Decision (b) 契約遵守)。OQ12 (projection rebuild hook) = out of scope、`opshub db migrate` の 0028 適用で 1 回 back-fill 自動、projection rebuild hook 化は Phase 16+ 候補に移送 (頻繁 rebuild の operation pattern が未顕在化)。
- **immutable migration 規範**: migration 0019 を遡及書き換えしない、新 migration 0028 で物理張り替え (`docs/phase-15-plan.md` §Alternatives #5 で却下根拠を記録)。downgrade で元 tokenizer に戻せる safety を担保。migration 0019 docstring に `.. note::` で 0028 supersede pointer を追加したのみ (S2 で確認、SQL body は immutable)。
- **MCP `search` tool 契約 (ADR-0022) は無変更**: `raw_query` は hard-coded `false` のまま。tokenizer 張り替えと LIKE fallback は SearchService 内部処理として透過し、MCP tool 契約 (引数 / response schema) は不変。アシスタント 14 Skill (`find-document` / `personal-brief` / `next-actions` / `meeting-prep` / `research` / etc.) は MCP `search` tool 1 箇所改善で透過的に恩恵を受ける。
- **cross-cutting fix の closeout**: S2 PR #363 で flag された `tests/unit/mcp/test_phase12_handlers.py::_bootstrap_fts_index()` (production 切替後も `unicode61 remove_diacritics 2` を seed していた divergence) は S3 PR #364 で `trigram` に同期した。これにより MCP search-handler unit tests が operator の見る MATCH semantics と一致するようになり、Phase 15 全体で production / migration / SearchService / MCP unit boundary が再整合した。
- **Phase 16+ outlook (`docs/phase-15-plan.md` §9)**: 形態素 tokenizer 採用 (Lindera / SudachiPy / MeCab) / dual index (unicode61 + trigram) / `opshub search rebuild-index` 専用 CLI / MCP `search` tool 契約改訂 (`raw_query` を operator 露出) / 検索結果の semantic re-rank (vector との hybrid score) / snippet() ハイライト / 検索クエリの NFKC 正規化 / 全角半角統一 は Phase 16+ で再評価。Phase 14 §9 から繰り越し (multi-machine sync / 能動性段階 1-4 / 画像 OCR / Drive Comments / Suggestions / Gmail / Calendar 添付の本文抽出 / 追加コネクタ / 外部書き戻し / Calendar instance 展開 projection / `ozzy-labs/skills` 配布完成) は本 Phase で touch せず Phase 16+ に維持。
- **関連 ADR**: ADR-0001 (Python Stack、cold-start 予算) / ADR-0020 (Full Local Content Retention、`sources.body` は本文の正規ストア、FTS5 は派生 index) / ADR-0022 (MCP Server Surface、`search` tool 契約 = `raw_query` hard-coded false、本 Phase では不変) / ADR-0028 (新規、本 Phase で起草) — FTS5 sources_fts tokenizer choice (trigram + short-query LIKE fallback)。
