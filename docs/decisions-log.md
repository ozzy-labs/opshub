...
- **Phase 15+ outlook**: opt-in OTel exporter (現状は structlog OTel GenAI naming のみ、exporter は `mcp-otel` extras で予定) / `--quiet` を rich progress と組み合わせた "ログ抑制 + 進捗だけ表示" モード / 秘書 14 Skill 側からのトラブルシュート手順 (skill 内エラー時に operator に `--debug` で再現を促す pattern) は Phase 15+ で再評価。本 epic は CLI / MCP のログ verbosity surface に scope を限定して締めた。
...
- **MCP `search` tool 契約 (ADR-0022) は無変更**: `raw_query` は hard-coded `false` のまま。tokenizer 張り替えと LIKE fallback は SearchService 内部処理として透過し、MCP tool 契約 (引数 / response schema) は不変。秘書 14 Skill (`find-document` / `personal-brief` / `next-actions` / `meeting-prep` / `research` / etc.) は MCP `search` tool 1 箇所改善で透過的に恩恵を受ける。
