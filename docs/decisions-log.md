...
- **R 不変条件 (6 件)**: R1 全ログ値が structlog redaction processor 経由 (`format_exc_info` の後・renderer の前)。R2 `--debug` の full traceback は `format_debug_traceback` でサニタイズ。R3 connector sync デフォルト = 例外型名のみ (`ConnectorSyncFailed.error_message` / `sync failed: <Type>` の **stderr** サマリは不変、成功時の `synced ...` は stdout のまま)、`--debug` 時のみ stderr に opt-in 追加。R4 既知トークン形状 (`sk-` / `ghp_` / `github_pat_` / `xox*-` / `AKIA` / `AIza` / JWT / `Bearer`) は全 verbosity で marker 化。R5 `--log-file` 指定時のファイル新規作成は `os.open(O_CREAT|O_WRONLY|O_APPEND, 0o600)`。R6 `opshub --help` cold-start ≤300 ms 維持 (`test_cli_imports.py` の whitelist に `opshub.core.logging` を持ち込まず、`cli/app.py` 内で lazy import)。
