...
## 35. Phase 15 着手 (Search 品質改善、ADR-0028、epic #338、2026-06-02)

Phase 14 closeout (§32、2026-05-31) を受けて、Phase 15+ outlook で forecast されていた「Search 品質改善 (FTS5 日本語 tokenizer 修正)」を Phase 15 として正式起票した (epic #338、`docs/phase-15-plan.md` を SSOT として起草)。Phase 10 で導入した `opshub search` (migration 0019、`sources_fts`) が日本語自然文 (例: 「boxの権限」「進捗記入」「依頼」) で 0 hit になる問題を、`sources_fts` の tokenizer を `unicode61 remove_diacritics 2` から FTS5 built-in `trigram` に張り替え + SearchService 層で 1-2 文字短クエリを `body LIKE '%q%'` にルーティングする fallback を入れて根治する。形 A (能動性なし) + 外部書き戻しなしを Phase 10〜14 から継承。
...
- **Wave 構成 (plan、`docs/phase-15-plan.md` §3)**: **Wave 1 (S1)** = ADR-0028 + `docs/phase-15-plan.md` + `docs/adr/README.md` 索引 + AGENTS.md Status 行への Phase 15 着手追記 (本 plan で実装変更なし、entry PR)。**Wave 2 (S2)** = `src/opshub/db/migrations/versions/0028_rebuild_sources_fts_trigram.py` 新規 (drop trigger 3 + drop `sources_fts` → create with `tokenize='trigram'` → back-fill from `sources.body` → re-create 3 triggers、downgrade 経路は元 tokenizer に復元)。**Wave 3 (S3)** = `src/opshub/services/search_service.py` 改修 (短クエリ判定 ≤2 文字 → `body LIKE '%q%' ESCAPE '\\'` path、`--raw` モードでは fallback 無し)。**Wave 4 (S4)** = closeout (docs / troubleshooting / AGENTS Phase status 行 → `Phase 1-15 complete` 反映、`opshub search --help` の `--raw` 説明更新)。
- **関連 PR**: S1 = ADR + plan + index、S2 = migration 0028、S3 = SearchService 改修、S4 = closeout (PR 番号は実装時点で確定。本表 [pr-number-forecast-not-canonical] により forecast は記載しない)。
- **immutable migration 規範**: migration 0019 を遡及書き換えしない、新 migration 0028 で物理張り替え (`docs/phase-15-plan.md` §Alternatives #5 で却下根拠を記録)。downgrade で元 tokenizer に戻せる safety を担保。
