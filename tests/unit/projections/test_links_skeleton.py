"""Unit tests for the Phase 8 step A2 ``LinksProjector`` skeleton.

These tests pin the A2 contract for
...
* The natural-key UNIQUE constraint and the two bidirectional
  ...
  separately).
* The :class:`~opshub.projections.links.LinksProjector` reducer is a
  no-op pending Phase 8 step B2 — applying any event (briefing,
  proposal, task, …) leaves the table empty and does not raise.
* :func:`opshub.projections.registry.all_projections` lists the new
...
The skeleton tests intentionally do NOT exercise the per-event
dispatch — that is Phase 8 B2's responsibility (which will add
``tests/unit/projections/test_links_extractor.py``). Locking the
no-op contract here means a regression where B2's logic accidentally
leaks into A2 (or a future refactor undoes the no-op) is caught at
review time.
"""
...
# ---- Phase 8 A2 skeleton: apply must be a no-op --------------------------


def test_linksprojector_apply_is_noop_skeleton(engine: Engine) -> None:
    """The A2 skeleton accepts any event and writes nothing.

    Phase 8 B2 replaces the reducer body with the dispatch table;
    until then the projector must be inert. Locking the no-op here
    means an accidental partial-implementation in B2 (or a future
    refactor that mixes A2 and B2 wiring) is caught before it can
    silently populate rows from an incomplete extraction path.
    """
    ...
    assert rows == [], (
        "Phase 8 A2 LinksProjector.apply must be a no-op; B2 lands the dispatch logic"
    )
