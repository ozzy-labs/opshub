...
# The five Phase 12 H1 (post-rename) secretary skills + the three
# Phase 12 H4 HITL write skills (inbox-triage / source-extract /
# meeting-followup). Phase 12 H1 renames the original brief / lookup
# pair to ``personal-brief`` and ``find-document``; ``next-actions`` /
# ``reply-draft`` / ``pr-review`` keep their names. H4 adds three new
# HITL write skills that all route through ``propose.generate`` +
# ``propose.apply`` (ADR-0016 改訂 §決定 (l)(b)).
